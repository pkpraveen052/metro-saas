...
from odoo.addons.metro_helpdesk.models.helpdesk_ticket import TICKET_PRIORITY
...
class HelpdeskTeam(models.Model):
    ...
    def _default_stage_ids(self):
        ...
        return [(0, 0, {'name': _('New'), 'sequence': 0, 'template_id': self.env.ref('metro_helpdesk.new_ticket_request_email_template', raise_if_not_found=False) or None})]

    def _default_domain_member_ids(self):
        return [('groups_id', 'in', self.env.ref('metro_helpdesk.group_helpdesk_user').id)]
    ...
    def _check_sla_group(self):
        for team in self:
            if team.use_sla and not self.user_has_groups('metro_helpdesk.group_use_sla'):
                self.env.ref('metro_helpdesk.group_helpdesk_user').write({'implied_ids': [(4, self.env.ref('metro_helpdesk.group_use_sla').id)]})
            if team.use_sla:
                ...
                if not self.search_count([('use_sla', '=', True)]):
                    self.env.ref('metro_helpdesk.group_helpdesk_user').write({'implied_ids': [(3, self.env.ref('metro_helpdesk.group_use_sla').id)]})
                    self.env.ref('metro_helpdesk.group_use_sla').write({'users': [(5, 0, 0)]})
            ...
            if team.use_rating:
                for stage in team.stage_ids:
                    if stage.is_close and not stage.fold:
                        stage.template_id = self.env.ref('metro_helpdesk.rating_ticket_request_email_template', raise_if_not_found= False)
    ...
    def retrieve_dashboard(self):
        ...
        user_uses_sla = self.user_has_groups('metro_helpdesk.group_use_sla') and\
        ...
        tickets = HelpdeskTicket.search_read([],['sla_deadline', 'open_hours', 'sla_reached_late', 'priority'])
        ...
        result['today']['success'] = "{:.2f}".format(result['today']['success'] * 100 / (result['today']['count'] or 1))
        result['7days']['success'] = "{:.2f}".format(result['7days']['success'] * 100 / (result['7days']['count'] or 1))
        result['my_all']['hours'] = "{:.2f}".format(result['my_all']['hours'] / (result['my_all']['count'] or 1))
        result['my_high']['hours'] = "{:.2f}".format(result['my_high']['hours'] / (result['my_high']['count'] or 1))
        result['my_urgent']['hours'] = "{:.2f}".format(result['my_urgent']['hours'] / (result['my_urgent']['count'] or 1))
    ...
    def _action_view_rating(self, period=False, only_my_closed=False):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("metro_helpdesk.rating_rating_action_helpdesk")
        action['domain'] = [('res_id', 'in', ticket_ids), ('rating', '!=', -1), ('res_model', '=', 'helpdesk.ticket'), ('consumed', '=', True)]
        return action

    def action_view_ticket(self):
        action = self.env["ir.actions.actions"]._for_xml_id("metro_helpdesk.helpdesk_ticket_action_team")
