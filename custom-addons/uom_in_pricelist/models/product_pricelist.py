...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(self, products_qty_partner, date=False, uom_id=False):
        ...
        for product, qty, partner in products_qty_partner:
            ...
            for rule in items:
                line_level_uom = self.env['uom.uom'].browse(qty_uom_id)
                if not rule._is_applicable_for(product, qty,line_level_uom):
                    continue
                if rule.compute_price == 'fixed':
                    # Use the rule's fixed price directly, assuming it's defined in rule.product_uom_id
                    # This condition is written when the product's base uom change to its other category uom
                    # and according to that uom get the unit price from pricelist and return it.
                    price = rule.fixed_price
                    # If the UoM from the rule is different than desired, convert it
                    if rule.product_uom_id and rule.product_uom_id != price_uom:
                        price = rule.product_uom_id._compute_price(price, price_uom)
                    # results[product.id] = (price, rule.id)
                    suitable_rule = rule
                    break  # Exit after matching first applicable rule
