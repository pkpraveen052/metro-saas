...
class Home(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if user_id.kitchen_screen_user == 'cook' and user_id.default_pos:
            ...
            #     [('config_id', '=', user_id.default_pos.id), ('state', '=', 'opening_control')]) #Metro Commented
            pos_session = request.env['pos.session'].sudo().search(
                [('company_id','=',user_id.company_id.id), ('config_id', '=', user_id.default_pos.id), ('state', '=', 'opened')], limit=1) #Metro added
            if not pos_session:
                pass
                #Metro Commented
