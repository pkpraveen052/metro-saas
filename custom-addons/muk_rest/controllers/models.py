...
class ModelController(http.Controller):
    ...
    def search_read(self, model, domain=None, fields=None, context=None, limit=80, offset=0, order=None, **kw):
        ...
        offset = offset and int(offset) or None
        model = request.env[model].with_context(ctx)
        result = model.sudo().search_read(domain, fields=fields, offset=offset, limit=limit, order=order)
        _logger.info("\n\nmodel ===")
        _logger.info(model)
        _logger.info(result)
        if str(model._name) == 'security.location':
            for dic in result:
                if dic.get('security_ids'):
                    security_persons = request.env['security.person'].browse(dic['security_ids']).mapped('name')
                    _logger.info(security_persons)
                    dic.update({'security_ids': security_persons})
