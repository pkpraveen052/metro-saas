        ...
        for move in self:
            if move.name == '/':
                ...
                if not move.transaction_type:
                    ...
                    sequence = self.env['ir.sequence'].with_context(ir_sequence_date=move.date)
                    if move.transaction_type == 'send':
                        move.name = sequence.next_by_code('account.move.spend')
                    else:
                        move.name = sequence.next_by_code('account.move.receive')
