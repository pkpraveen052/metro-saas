...
#
# Inherited Functions from "base/models/res_users.py" for manipulating boolean and selection pseudo-fields
#
...
class GroupsView(models.Model):
    ...
    def _update_user_groups_view(self):
        """
            Overidden method from the base/models/res_users.py to hide the Administration Section.
            Also hides the Point of Sale Group selection field to those users who has no group set.
            Also hides the CRM Group selection field to those users who has no group set.
        """
        ...
        if self._context.get('install_filename') or self._context.get(MODULE_UNINSTALL_FLAG):
            ...
            for app, kind, gs, category_name in sorted_tuples:  # we process the user type first
                ...
                if app.xml_id == 'base.module_category_user_type':
                    ...
                    # Custom Code starts
                    if app.xml_id == 'base.module_category_sales_point_of_sale' and self.env['ir.module.module'].search([('name', '=', 'metroerp_pos'), ('state', '=', 'installed')]):
                        attrs['attrs'] = str({'invisible': [('enable_pos', '=', False)]})
                    if app.xml_id == 'base.module_category_sales_crm' and self.env['ir.module.module'].search([('name', '=', 'metroerp_crm_enhancement'), ('state', '=', 'installed')]):
                        attrs['attrs'] = str({'invisible': [('enable_crm', '=', False)]})
                    if app.xml_id == 'metroerp_userguide.module_category_for_user_guide' and self.env['ir.module.module'].search([('name', '=', 'metroerp_userguide'), ('state', '=', 'installed')]):
                        attrs['attrs'] = str({'invisible': [('company_id', '!=', 1)]})
                    if app.xml_id == 'base.module_metroerp_service_management' and self.env['ir.module.module'].search([('name', '=', 'metroerp_service_management'), ('state', '=', 'installed')]):
                        attrs['attrs'] = str({'invisible': [('enable_service', '=', False)]})
                    # Custom Code ends
                    ...
                    for g in gs:
                        ...
                        if g == group_no_one:
                            ...
                            if app.xml_id == 'base.module_category_hidden' and g.id == self.env.ref('metroerp_saas_customization.accounting_partner').id:
                                attrs['attrs'] = str({'invisible': [('enable_ap', '=', False)]})
                            if app.xml_id == 'base.module_category_hidden' and g.id == self.env.ref('metroerp_saas_customization.group_restrict_user').id:
                                attrs['attrs'] = str({'invisible': [('restrict_ap', '=', False)]})
            ...
            for xml_cat in sorted(xml_by_category.keys(), key=lambda it: it[0]):
                ...
                # Custom Code starts
                master_category_names = ["Administration", "Developer Tools"]
                if master_category_name in master_category_names:
                    self.env['res.users'].search([])._compute_is_admin()
                    temp_groups = "base.group_erp_manager,base.group_system"
                    xml2.append(E.group(*(xml_by_category[xml_cat]), col="2", string=master_category_name,
                                        groups=temp_groups))
                elif master_category_name == "User Guide":
                    xml2.append(E.group(*(xml_by_category[xml_cat]), col="2", string=master_category_name,
                    attrs=str({'invisible': [('company_id', '!=', 1)]})))
