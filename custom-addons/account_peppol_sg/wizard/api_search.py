...
class PeppolIdentofierSearch(models.TransientModel):
        ...
        if response.status_code == 200:
            ...
            for match in data.get('matches'):
                line = (0, 0, {
                    ...
                    'reg_date': datetime.strptime(match.get('entities')[0].get('regDate'), '%Y-%m-%d') if 'regDate' in
                                                                                                          match.get(
                                                                                                              'entities')[
                                                                                                              0] else None,
    ...
    def action_search_api_customer(self):
        return self.action_search_api('account_peppol_sg.peppol_search_data_wizard_form_view_sales')

    def action_search_api_vendor(self):
        return self.action_search_api('account_peppol_sg.peppol_search_data_wizard_form_view_puchases')
