...
class GSTReturnsF7(models.Model):
    _name = "gst.returns.f7"
    _description = "Past GST Return (GST F7)"
    _inherit = ['mail.thread', 'mail.activity.mixin', 'base.gst.accounts']
    _order = "create_date desc"
    _rec_name = "id"

    @api.constrains('contactEmail')
    ...
    @api.constrains('badDebtReliefClaimAmt')
    def _check_badDebtReliefClaimAmt(self):
    ...
    @api.constrains('preRegistrationClaimAmt')
    def _check_preRegistrationClaimAmt(self):
    ...
    @api.depends('outputTaxDue', 'inputTaxRefund') #'prevGSTPaid','netDifference'
    ...
    @api.constrains('defImpPayableAmt','defTotalGoodsImp')
    def constrain_sn_20(self):
    ...
    def create(self, vals):
        vals['state'] = 'draft'
        obj = super(GSTReturnsF7, self).create(vals)
        if obj.gst_f5f8_id:
            obj.gst_f5f8_id.write({'gst_f7_id': obj.id})
        return obj
    ...
    def action_ready_submit(self):
        ...
        if (self.totTaxPurchase < self.inputTaxRefund) and self.validation_sn11_done == False: #done
            return {
                'name': _('GST Returns Prompt'),
                'view_mode': 'form',
                'res_model': 'gst.returns.prompt.wizard',
        ...
        if (self.inputTaxRefund != 0 and self.totTaxPurchase == 0) and self.validation_sn12_done == False: #done
            return {
                'name': _('GST Returns Prompt'),
                'view_mode': 'form',
                'res_model': 'gst.returns.prompt.wizard',
        ...
        if (self.totValueScheme != 0 and self.totTaxPurchase < self.totValueScheme) and self.validation_sn14_done == False:
            ...
            # Pintu ---------------
        # if (self.inputTaxRefund != 0 and self.totValueScheme == self.totTaxPurchase) and self.validation_sn16_done == False:
        #     return {
        #         'name': _('GST Returns Prompt'),
        #         'view_mode': 'form',
        #         'res_model': 'gst.returns.prompt.wizard',
        #         'view_id': self.env.ref('metroerp_iras.gst_returns_prompt_wizard_view_f7_sn16').id,
        #         'type': 'ir.actions.act_window',
        #         'target': 'new'
        #     }
        # else:
        #     self.validation_sn16_done = True

        # if (self.totValueScheme != 0 and self.totTaxPurchase < self.totValueScheme) and self.validation_sn17_done == False:
        #     return {
        #         'name': _('GST Returns Prompt'),
        #         'view_mode': 'form',
        #         'res_model': 'gst.returns.prompt.wizard',
        #         'view_id': self.env.ref('metroerp_iras.gst_returns_prompt_wizard_view_f7_sn17').id,
        #         'type': 'ir.actions.act_window',
        #         'target': 'new'
        #     }
        # else:
        #     self.validation_sn17_done = True
    ...
    def action_click_me_edit(self):
        self.write({
            ...
            'validation_sn15_done': False,
            'validation_sn16_done': False,
            'validation_sn17_done': False,
    ...
    def write(self, vals):
        ...
        return super(GSTReturnsF7, self).write(vals)
    ...
    def _compute_net_gstpaid(self):
        self.netGSTPaid = self.outputTaxDue - self.inputTaxRefund

    @api.depends('netGSTPaid','prevGSTPaid')
    def _compute_net_difference(self):
        self.netDifference = self.netGSTPaid - self.prevGSTPaid
    ...
    taxRefNo = fields.Char(string="Tax Reference Number of Company/Business", required=True, size=30, tracking=True, default=lambda self: self.env.company.vat)
    formType = fields.Selection([("F7","F7")],string="Form Type of the GST Return", required=True, tracking=True, default='F7')
    ...
    totStdSupply = fields.Float(string="Total value of standard-rated supplies", digits=(14, 0), required=True, tracking=True)
    totZeroSupply = fields.Float(string="Total value of zero-rated supplies", digits=(14, 0), required=True, tracking=True)
    totExemptSupply = fields.Float(string="Total value of exempt supplies", digits=(14, 0), required=True, tracking=True)
    totalSupply = fields.Float(string="Total Supply", compute="_compute_supply", store=True, digits=(14, 0), tracking=True)
    totTaxPurchase = fields.Float(string="Total revised value of taxable purchases", digits=(14, 0), required=True, tracking=True)
    outputTaxDue = fields.Float(string="Revised output tax", digits=(14, 2), required=True, tracking=True)
    inputTaxRefund = fields.Float(string="Revised input tax and refunds claimed", digits=(14, 2), required=True, tracking=True)
    netGSTPaid = fields.Float(string="Equals: Net GST to be paid to/claimed from IRAS", compute="_compute_net_gstpaid", store=True, digits=(14, 2), tracking=True)
    prevGSTPaid = fields.Float(string="Net GST paid/claimed previously for this accounting period", digits=(14, 2), required=True, tracking=True)
    netDifference = fields.Float(string="Difference to be paid to / claimed from IRAS", compute="_compute_net_difference", store=True, digits=(14, 2), tracking=True)
    totalTax = fields.Float(string="Total Tax", digits=(14, 2), compute="_compute_tax", store=True, tracking=True)
    totValueScheme = fields.Float(string="Total value of goods imported under import GST suspension schemes", digits=(14, 0), required=True, tracking=True, copy=False)
    touristRefundChk = fields.Boolean(string="Did you claim for GST you had refunded to tourists?", required=True, tracking=True, copy=False)
    touristRefundAmt = fields.Float(string="Tourist Refund Amount", digits=(14, 2), tracking=True, copy=False)
    badDebtChk = fields.Boolean(string="Did you make any bad debt relief claims and/or refund for reverse charge transactions?", required=True, tracking=True, copy=False)
    ...
    preRegistrationChk = fields.Boolean(string=" Did you make any pre-registration claims?", required=True, tracking=True, copy=False)
    preRegistrationClaimAmt = fields.Float(string="Pre-registration Claims Amount", digits=(14, 2), tracking=True, copy=False)
    ...
    RCLVGChk = fields.Boolean(string="Did you import services and/or low-value goods subject to GST under reverse charge?", required=True, tracking=True, copy=False)
    totImpServLVGAmt = fields.Float(string="Value of imported services and/or low-value goods subject to reverse charge", digits=(14, 0), required=True, tracking=True, copy=False)
    ...
    OVRRSChk = fields.Boolean(string="Did you operate an electronic marketplace to supply remote services (includes digital and nondigital services) subject to GST on behalf of thirdparty suppliers?", required=True, tracking=True, copy=False)
    ...
    RedlvrMktOprLVGChk = fields.Boolean(string="Did you operate as a redeliverer, or an electronic marketplace to supply imported low-value goods subject to GST on behalf of third-party suppliers?", reequired=True, tracking=True, copy=False)
    ...
    OwnImpLVGChk = fields.Boolean(string="Did you make your own supply of imported low-value goods that is subject to GST?", required=True, tracking=True, copy=False)
    totOwnImpLVGAmt = fields.Float(string="Value of own supply of imported low-value goods", digits=(14, 0), tracking=True, copy=False)
    defImpPayableAmt = fields.Float(string="Revised deferred import GST payable", digits=(14, 2), required=True, tracking=True, copy=False)
    prevDefImpGSTPayable = fields.Float(string="Deferred import GST payable previously declared for this accounting period", digits=(14, 2), required=True, tracking=True, copy=False)
    diffDefImpGSTPayable = fields.Float(string="Difference in deferred import GST payable", digits=(14, 2), required=True, tracking=True, copy=False)
    diffDefImpNetGST = fields.Float(string="Difference in Net GST (per box 10 above)", digits=(14, 2), tracking=True, copy=False)    
    diffDefTotalTaxAmt = fields.Float(string="Difference in total tax to be paid to /claimed from IRAS", digits=(14, 2), tracking=True, copy=False)
    defTotalTaxAmt = fields.Float(string="Revised total tax to be paid to /claimed from IRAS", digits=(14, 2), tracking=True, copy=False)
    defTotalGoodsImp = fields.Float(string="Total value of goods imported under Import GST Deferment Scheme", digits=(15, 0), tracking=True, copy=False)
    descriptionOfError = fields.Text(string="Description of Errors", size=200, tracking=True)
    declareTrueCompleteChk = fields.Boolean(string="I declare that the information provided in this return is true and complete.", required=True, tracking=True, copy=False, default=True)
    declareIncRtnFalseInfoChk = fields.Boolean(string="I understand that penalties may be imposed for the submission of an incorrect return and/or provision of false information to the Comptroller of GST.", required=True, tracking=True, copy=False, default=True)
    declarantDesgtn = fields.Char(string="Designation", size=60, required=True, tracking=True, default=lambda self: self.env.user.function)
    contactPerson = fields.Char(string="Contact Person", size=100, required=True, tracking=True, default=lambda self: self.env.user.name)
    contactNumber = fields.Char(string="Contact Tel No. (+65)", size=8, required=True, tracking=True, default=lambda self: self.env.user.phone)
    contactEmail = fields.Char(string="Contact Email", size=50, required=True, tracking=True, default=lambda self: self.env.user.login)
    ...
    json_response = fields.Text(string="JSON Response", tracking=True, readonly=True, copy=False)
    ...
    validation_sn15_done = fields.Boolean(copy=False)
    validation_sn16_done = fields.Boolean(copy=False)
    validation_sn17_done = fields.Boolean(copy=False)
    gst_f5f8_id = fields.Many2one("gst.returns.f5f8", "GST Returns F5 and F8")
    ...
    def action_submit(self):
        ...
        if not self.manager_signature_done:
            return {
                ...
                'view_id': self.env.ref('metroerp_iras.gst_returns_signature_f7_form').id,
                'type': 'ir.actions.act_window',
                'target': 'new',
                'res_id': self.id,
            }
        ...
        state += '_' + str(config_params.get_param('web.base.url')) + '_' + str(self.id) + '_' + 'GSTF7SubCP' + '_' + str(self.env.ref('metroerp_iras.gst_returns_f7_action').id)
        ...
        url = "{}?scope={}&callback_url={}&tax_agent={}&state={}".format(url, params['scope'], params['callback_url'],
                                                                         str(params['tax_agent']).lower(),
                                                                         params['state'])
        print("url ===",url)
        ...
        if res_data.get('httpCode'):
            ...
            if res_data['returnCode'] in ["20", "30"]:
                return {
                    ...
                    'tag': 'display_notification',
                    'params': {
                        'title': res_data['info'].get('message') and res_data['info']['message'] or (
                                    'Error with returnCode' + res_data['returnCode']),
                        'message': json.dumps(res_data),
                        'sticky': False,
                    }
                }
    ...
    def generate_json_request(self):
        ...
        data = {
             ...
             "prevGSTPaid":self.prevGSTPaid,
             "netDifference":self.netDifference
             },
             "schemes":{
            ...
            "igdScheme": {
                ...
                "prevDefImpGSTPayable": self.prevDefImpGSTPayable,
                "diffDefImpGSTPayable": self.diffDefImpGSTPayable,
                "defTotalGoodsImp": self.defTotalGoodsImp
            },
            "ErrorDescription": {
                "descriptionOfError": self.descriptionOfError
            },
            "declaration": {
            ...
            "reasons": {
                ...
                "grp1OtherReasons": self.grp1OtherReasons if self.grp1OtherReasons else "",
                ...
                "grp2OtherReasons": self.grp2OtherReasons if self.grp2OtherReasons else "",
                "grp3CreditNotesChk": self.grp3CreditNotesChk,
                "grp3OtherReasonsChk": self.grp3OtherReasonsChk,
                "grp3OtherReasons": self.grp3OtherReasons if self.grp3OtherReasons else ""
            },
        }
    ...
    def autosubmit_gst_data(self):
        ...
        url = config_params.get_param('gstreturns_f7_endpoint')
        ...
        if res_data.get('returnCode'):
            if res_data['returnCode'] == '10':
                ...
                self.write({
                    ...
                    # 'netGSTPaid': res_data["data"]["taxes"]["netGSTPaid"],
                    'diffDefImpNetGST': res_data["data"]["igdScheme"]["diffDefImpNetGST"],
                    'diffDefTotalTaxAmt': res_data["data"]["igdScheme"]["diffDefTotalTaxAmt"],
                    'defTotalTaxAmt': res_data["data"]["igdScheme"]["defTotalTaxAmt"],
                    'defTotalGoodsImp': res_data["data"]["igdScheme"]["defTotalGoodsImp"],
    ...
    def fetch_details_OLD(self):
        config_params = self.env['ir.config_parameter'].sudo()
        target_move = self.company_id.target_moves_f7
