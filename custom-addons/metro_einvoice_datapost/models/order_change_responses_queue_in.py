...
class OrderChangeResponsesQueueIn(models.Model):
    _name = "order.change.responses.queue.in"
    ...
    order_queue_out_id = fields.Many2one("order.change.queue.out", string="Related Document", readonly=True)
                ...
                if x.tag == cac + 'OrderReference':  # <cac:OrderReference>
                    for child in x:
                        if child.tag == cbc + 'ID':
                            order_no = child.text
                            print('\n\n\norder_no', order_no)
                            queue_obj = self.env['order.change.queue.out'].search([('order_id.name', '=', child.text)],
                                                                            limit=1)
                            if queue_obj:
    ...
    def query_change_order_res_receive_documents(self):
