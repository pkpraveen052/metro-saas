...
class OrderBalanceIn(models.Model):
    _name = "order.balance.in"
    _description = "Incoming Balance Queue"
    _rec_name = "order_no"
    ...
    document_ids = fields.Many2many(
        'ir.attachment', 'unmapped_balance_order_docs_rel', 'order_balance_id', 'attachment_id',
    ...
    order_line_ids = fields.One2many("incoming.balance.order.lines", "parent_id", "Order Lines")
    ...
    def _compute_order_response_count(self):
        for rec in self:
            order_responses_count = self.env['order.balance.responses.queue.out'].search_count(
    ...
    def action_view_order_response_queue_out(self):
        ...
        return {
            ...
            'name': name,
            'res_model': 'order.balance.responses.queue.out',
    ...
    def unlink(self):
        ...
        return super(OrderBalanceIn, self).unlink()
...
class IncomingBalanceOrderLines(models.Model):
    _name = "incoming.balance.order.lines"
    _description = "Incoming Balance Order Lines"

    parent_id = fields.Many2one('order.balance.in', 'Parent')
