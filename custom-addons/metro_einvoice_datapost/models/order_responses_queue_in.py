...
class OrderResponsesQueueIn(models.Model):
    _name = "order.responses.queue.in"
    ...
    order_queue_out_id = fields.Many2one("order.queue.out", string="Related Document", readonly=True)
            ...
            for x in myroot:
                ...
                if x.tag == cac + 'OrderReference':  # <cac:OrderReference>
                    for child in x:
                        if child.tag == cbc + 'ID':
                            order_no = child.text
                            queue_obj = self.env['order.queue.out'].search([('order_id.name', '=', child.text)],
                                                                            limit=1)
                            if queue_obj:
    ...
    def _create_incoming_order_response(self, resp_dic, api_dic, company_id, access_point_id):
        print(self.search([('extracted_docno', '=', resp_dic.get('documentNo', '').strip())]))
        ...
        try:
            response = requests.get('%s/api/business/%s/order-responses/%s' % (api_dic['base_url'],
                                                                        api_dic['api_version'],
        ...
        for access_point in self.env['peppol.access.point.sg'].sudo().search([('company_id', '=', self.env.company.id)]):
            ...
            try:
                ...
                response = requests.get(
                    '%s/api/business/%s/order-responses?receiver=%s' % (base_url, api_version, receiver_peppol_id),
                    headers=headers,
                    timeout=int(timeout))
                print('%s/business/%s/order-responses.json?type=2&receiver=%s' % (base_url, api_version, receiver_peppol_id))
                status_code = response.status_code
                try:
                    response_data = json.loads(response.text)
                except:
