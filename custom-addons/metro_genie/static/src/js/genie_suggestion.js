...
//odoo.define('metro_genie.dashboard_form_handler', function (require) {
//    "use strict";
//
...
//
//    const GenieDashboardFormController = FormController.extend({
//        start: function () {
//            const self = this;
//            return this._super.apply(this, arguments).then(function () {
//                console.log("✅ Genie Dashboard FormView Initialized");
//
//                // Attach click handler AFTER rendering
//                setTimeout(() => {
//                    self.$('.genie-nav-btn').off('click').on('click', function (e) {
//                        e.preventDefault();
//                        const suggestion = $(this).data('suggestion');
//                        console.log("🟢 Clicked:", suggestion);
//
//                        // Call custom route, or trigger action by name or id
//                        self._rpc({
//                            model: 'metro.genie.suggestion',
//                            method: 'get_group_filtered_suggestions',
//                        }).then(data => {
//                            const match = data.find(d => d.name === suggestion);
//                            if (match && match.window_action_id) {
//                                self.do_action(match.window_action_id);
//                            } else if (match && match.menu_id) {
//                                self.do_action({
//                                    type: 'ir.actions.client',
//                                    tag: 'reload',
//                                });
//                                console.warn("🔶 Menu action fallback not handled.");
//                            } else {
...
//    });
//
//    const GenieDashboardView = FormView.extend({
//        config: _.extend({}, FormView.prototype.config, {
//            Controller: GenieDashboardFormController,
//        }),
//    });
//
//    viewRegistry.add('genie_dashboard_form', GenieDashboardView);
//});
...
odoo.define('metro_genie.dashboard_form_handler', function (require) {
    ...
    const GenieDashboardFormController = FormController.extend({
        start: function () {
            ...
            return this._super.apply(this, arguments).then(function () {
                console.log("✅ Genie Dashboard FormView Initialized");
            self.$('.genie-nav-btn').off('click').on('click', function () {
                const suggestion = $(this).data('suggestion');
                console.log('suggestion', suggestion)
                    self._rpc({
                        ...
                        const match = data.find(d => d.name === suggestion && d.is_button);
                        console.log('match', match)
                        if (!match) {
                            alert("No Action found!");
                        }
                        const urlParts = [];
                        if (match.window_action_id) urlParts.push(`action=${match.window_action_id}`);
                        if (match.client_actions_id) urlParts.push(`action=${match.client_actions_id}`);
                        if (match.menu_id) urlParts.push(`menu_id=${match.menu_id}`);
                        if (match.model) urlParts.push(`model=${match.model}`);
                        if (match.view_types) urlParts.push(`view_type=${match.view_types}`);
                        const url = '/web#' + urlParts.join('&');
