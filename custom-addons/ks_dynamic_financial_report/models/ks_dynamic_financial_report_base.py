...
from odoo.tools import date_utils, get_lang, ustr, DEFAULT_SERVER_DATE_FORMAT
...
from odoo.http import request #TODO
import time
...
class ks_dynamic_financial_base(models.Model):
    ...
    def ks_date_filter(self):
        if self.ks_comparison_range:
            if self.display_name == 'Tax Report':
                return {'ks_process': 'range', 'ks_filter': 'this_quarter'}
            else:
                return {'ks_process': 'range', 'ks_filter': 'this_year'}
        else:
            if self.display_name == 'Trial Balance':
                return {'ks_process': 'range', 'ks_filter': 'this_year'}
        ...
        for ks_report in ks_df_reports:
            ...
            if ks_report.ks_df_report_account_type == 'accounts':
                ks_res[ks_report.id]['account'] = self.sudo()._ks_compute_account_balance(
                                                        ks_report.sudo().ks_df_report_account_ids.filtered(lambda a: a.company_id.id == company_id),
                ...
                if self.ks_df_report_account_report_ids != \
                    ...
                    ks_res2 = self._ks_calculate_report_balance(ks_report.ks_df_report_account_report_ids,
                                                                ks_df_informations) # .with_context(ks_df_informations.get('ks_filter_context'))
                    for key, ks_value in ks_res2.items():
                        for field in ks_fields:
                            ks_res[ks_report.id][field] += ks_value[field]

            elif ks_report.ks_df_report_account_type == 'total':
                custom_reports = self.env['ks.dynamic.financial.reports']
                if ks_report.ks_custom_report_1 and ks_report.ks_custom_report_2:
                    custom_reports = ks_report.ks_custom_report_1 + ks_report.ks_custom_report_2
                else:
                    custom_reports = ks_report.ks_children_id
                ks_res2 = self.sudo()._ks_calculate_report_balance(custom_reports, ks_df_informations)
                for key, ks_value in ks_res2.items():
                    for field in ks_fields:
                        ks_res[ks_report.id][field] += ks_value[field]
    ...
    def _ks_compute_account_balance(self, accounts, ks_df_informations, prv_year_dates=False, current_year=False, ks_report=None):
        ...
        # Below lines of code added by dhaneswar    
        self = self.with_context(
            company_id=ks_df_informations.get('company_id'),
            account_ids=accounts  # Ensure this is a recordset of accounts
        )
        ...
        if accounts:
            ...
            for row in self.env.cr.dictfetchall():
                ...
                if self.ks_name == 'Balance Sheet':
                    ...
                    elif ks_report.ks_parent_id and ks_report.ks_parent_id.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_liability0').id:
                        _logger.info("--------")
                        _logger.info(ks_report)
                        _logger.info(ks_report.ks_name)
                        row['balance'] = 0 - row['balance']
                    ks_res[row['id']] = row
                else:
                    row['balance'] = 0 - row['balance']
    ...
    def ks_fetch_report_account_lines(self, ks_df_informations,offset={}):
        ...
        def sub_ks_fetch_report_account_lines(self, res, ks_df_informations, ks_child_reports):
            initial_date_to = ks_df_informations.get('ks_filter_context')['date_to']

            new_ks_df_informations = ks_df_informations
            new_ks_df_informations2 = ks_df_informations       
            
            res2 = False
            if new_ks_df_informations2.get('ks_filter_context', False) and not new_ks_df_informations2['ks_filter_context'].get('date_from', False) and new_ks_df_informations2['ks_filter_context'].get('date_to', False):
                filter_date_to = datetime.datetime.strptime(new_ks_df_informations2['ks_filter_context']['date_to'], DEFAULT_SERVER_DATE_FORMAT)
                company_fiscalyear_dates = self.env.company.compute_fiscalyear_dates(filter_date_to)
                date_from_var = company_fiscalyear_dates['date_from']   

                new_ks_df_informations2['ks_filter_context']['date_from'] = date_from_var.strftime(DEFAULT_SERVER_DATE_FORMAT) 
                new_ks_df_informations['ks_filter_context']['date_from'] = new_ks_df_informations2['ks_filter_context']['date_from']

                res2 = self.with_context(new_ks_df_informations2.get('ks_filter_context'))._ks_calculate_report_balance(
                    ks_child_reports, new_ks_df_informations2)

            current_year_earnings, previous_year_earnings = dict((k, 0.0) for k in ['credit','debit','balance']), dict((k, 0.0) for k in ['credit','debit','balance'])
            retained_earnings_obj = False
            equity_obj = False           

            for d_id, dic in res.items():
                d_obj = self.env['ks.dynamic.financial.reports'].browse(d_id)
                if d_obj.x_is_current_year:
                    if res2 and d_obj.id in res2:
                        new_dic = {'credit': res2[d_obj.id]['credit'], 'debit': res2[d_obj.id]['debit'], 'balance': res2[d_obj.id]['balance'] * -1}
                    else:
                        new_dic = {'credit': dic['credit'], 'debit': dic['debit'], 'balance': dic['balance'] * -1}
                    current_year_earnings = new_dic
                    res.update({d_id: new_dic})                
                elif d_obj.x_is_previous_year:
                    filter_ctx_dic = new_ks_df_informations.get('ks_filter_context')
                    if filter_ctx_dic.get('date_from'):                        
                        prev_earnings_end_date = datetime.datetime.strptime(filter_ctx_dic.get('date_from'), DEFAULT_SERVER_DATE_FORMAT) - datetime.timedelta(days=1)
                        new_date_to =  (prev_earnings_end_date).strftime(DEFAULT_SERVER_DATE_FORMAT)

                        filter_ctx_dic.update({
                            'date_from': str(d_obj.x_previous_earnings_startdate), 
                            'date_to': str(new_date_to)})
                    else:
                        filter_ctx_dic.update({
                            'date_from': str(d_obj.x_previous_earnings_startdate),
                            'date_to': False})

                    new_ks_df_informations['ks_filter_context'] = filter_ctx_dic

                    new_ks_child_reports = d_obj.ks_df_report_account_report_ids._ks_fetch_children_by_order()

                    pl_res = d_obj.with_context(new_ks_df_informations.get('ks_filter_context'))._ks_calculate_report_balance(
                        new_ks_child_reports, new_ks_df_informations)

                    # print("\n\n====================================================\n\npl_res ===",pl_res)
                    dic = pl_res.get(d_obj.ks_df_report_account_report_ids.id)
                    new_dic = {'credit': dic['credit'], 'debit': dic['debit'], 'balance': dic['balance']}

                    retaining_account = request.env['account.account'].search([('company_id','=',ks_df_informations['company_id']),('user_type_id.name', '=', 'Retained Earnings')], limit=1)

                    args = [
                        ('account_id', '=', retaining_account.id),
                        ('date', '>=', filter_ctx_dic['date_from']),
                    ...
                    if ks_df_informations.get('ks_posted_entries') and not ks_df_informations.get('ks_unposted_entries'):
                        ...
                        args += [('move_id.state','=','draft')]
                    else:
                        args += [('move_id.state','in', ('posted', 'draft'))]
                    retained_move_lines = request.env['account.move.line'].search(args)
                    retained_total_debit = sum(line.debit for line in retained_move_lines)
                    retained_total_credit = sum(line.credit for line in retained_move_lines)
                    retained_balance = retained_total_debit - retained_total_credit

                    new_dic = {'credit': dic['credit'] + retained_total_debit, 'debit': dic['debit'] + retained_total_credit, 'balance': dic['balance'] + (retained_balance * -1)}

                    previous_year_earnings = new_dic
                    res.update({d_id: new_dic})
                elif d_obj.ks_name == 'EQUITY':
                    equity_obj = d_obj
                else:
                    if d_obj.ks_children_id.filtered(lambda x: x.x_is_current_year or x.x_is_previous_year):
                        retained_earnings_obj = d_obj
                        new_dic = {'credit': dic['credit'], 'debit': dic['debit'], 'balance': dic['balance'] * -1}
                        res.update({d_id: new_dic})


            if retained_earnings_obj and retained_earnings_obj.ks_children_id:

                retained_child_objs = retained_earnings_obj.ks_children_id
                retained_child_ids = retained_child_objs.sorted(key=lambda r: r.ks_sequence).ids
                
                if retained_earnings_obj.ks_df_report_account_type == 'total':
                    retained_credit, retained_debit, retained_balance = 0, 0, 0
                    for child_id in retained_child_ids:
                        retained_credit += res[child_id]['credit']
                        retained_debit += res[child_id]['debit']
                        retained_balance += res[child_id]['balance']
                    retained_dic = res[retained_earnings_obj.id]
                    retained_dic.update({'credit': retained_credit, 'debit': retained_debit, 'balance': retained_balance})
                    res[retained_earnings_obj.id] = retained_dic

                elif retained_earnings_obj.ks_df_report_account_type == 'subtract':
                    retained_credit, retained_debit, retained_balance = [], [], []
                    for child_id in retained_child_ids:
                        retained_credit.append(res[child_id]['credit'])
                        retained_debit.append(res[child_id]['debit'])
                        retained_balance.append(res[child_id]['balance'])
                    retained_credit = reduce(lambda x,y: x-y, retained_credit)
                    retained_debit = reduce(lambda x,y: x-y, retained_debit)
                    retained_balance = reduce(lambda x,y: x-y, retained_balance)

                    retained_dic = res[retained_earnings_obj.id]
                    retained_dic.update({'credit': retained_credit, 'debit': retained_debit, 'balance': retained_balance})
                    res[retained_earnings_obj.id] = retained_dic

            if equity_obj and equity_obj.ks_children_id:
                equity_child_objs = equity_obj.ks_children_id
                equity_child_ids = equity_child_objs.sorted(key=lambda r: r.ks_sequence).ids
                
                if equity_obj.ks_df_report_account_type == 'total':
                    equity_credit, equity_debit, equity_balance = 0, 0, 0
                    for child_id in equity_child_ids:
                        equity_credit += res[child_id]['credit']
                        equity_debit += res[child_id]['debit']
                        equity_balance += res[child_id]['balance']
                    equity_dic = res[equity_obj.id]
                    equity_dic.update({'credit': equity_credit, 'debit': equity_debit, 'balance': equity_balance})
                    res[equity_obj.id] = equity_dic

                elif equity_obj.ks_df_report_account_type == 'subtract':
                    equity_credit, equity_debit, equity_balance = [], [], []
                    for child_id in equity_child_ids:
                        equity_credit.append(res[child_id]['credit'])
                        equity_debit.append(res[child_id]['debit'])
                        equity_balance.append(res[child_id]['balance'])
                    equity_credit = reduce(lambda x,y: x-y, equity_credit)
                    equity_debit = reduce(lambda x,y: x-y, equity_debit)
                    equity_balance = reduce(lambda x,y: x-y, equity_balance)
        ...
        if self.ks_name == 'Balance Sheet':
            res = sub_ks_fetch_report_account_lines(self, res, ks_df_informations, ks_child_reports)  
        ...
        if len(ks_df_informations.get('ks_differ')['ks_intervals']):
            for rec in ks_df_informations.get('ks_differ')['ks_intervals']:
                ...
                if self.ks_name == 'Balance Sheet':
                    ks_comparison_res1 = self.with_context(
                        ks_df_informations.get('ks_diff_filter_context'))._ks_calculate_report_balance(ks_child_reports,
                                                                                                       ks_df_informations)
                    ks_df_informations['ks_filter_context'] = ks_comp_filter_context
                    ks_comparison_res = sub_ks_fetch_report_account_lines(self, ks_comparison_res1, ks_df_informations, ks_child_reports)  
                else:
            ...
            if res[report.id].get('account'):
                ...
                for account_id, ks_value in res[report.id]['account'].items():
                        ...
                        if len(ks_df_informations['ks_differ']['ks_intervals']):
                            for rec_inter in ks_df_informations['ks_differ']['ks_intervals']:
                                ks_vals['balance_cmp']['comp_bal_' + rec_inter['ks_string']] = \
                                    ks_value['comp_bal_' + rec_inter['ks_string']] * int(report.ks_report_line_sign) # Metro Code 8Apr25
                    ...
                    if not currency_id.is_zero(ks_vals['balance']):
                        flag = True
                    # Metro Code starts
                    if 'balance_cmp' in ks_vals and any(value != 0 for value in ks_vals['balance_cmp'].values()):
                        flag = True
                    # Code ends
                    if flag:
    ...
    def ks_process_general_ledger(self, ks_df_informations,offset={}):
        ...
        if self.env.context.get('OFFSET',None):
            ...
            ks_account_ids = self.env['account.account'].sudo().search(search_query)
        ks_move_lines = {
            x.code: {
                'name': x.name,
                'code': x.code,
        ...
        for ks_account in ks_account_ids:
            ...
            for ks_row in cr.dictfetchall():
                ...
                if ks_currency.is_zero(ks_row['debit']) and ks_currency.is_zero(ks_row['credit']) and ks_currency.is_zero(ks_move_lines[ks_account.code]['initial_balance']):
    ...
    def ks_process_trial_balanc_OLD_19Aug25(self, ks_df_informations):
        print("\n\nks_process_trial_balance() >>>>>>>>>>>",ks_df_informations)
        print("\n\n",self._context)
        ks_domain = False
        ksaccount_ids = []
        if ks_df_informations:
            ...
            # ks_account_ids = ks_account_ids.filtered(lambda x: x.id == 14495)
            ks_move_lines = {x.code: {'name': x.name, 'code': x.code, 'id': x.id,
                                      ...
                                      'balance_carry_fwd': True,
                                      'comparision': {ks_df_informations['date'].get('ks_string'):{'initial_balance': 0.0,'debit': 0.0, 'credit': 0.0, 'balance': 0.0, 'ks_string': ks_df_informations['date'].get('ks_string')}}} for x in
            ...
            subtotal_comparision = {
                ks_df_informations['date'].get('ks_string'): 
                    {
                        'ks_total_deb': 0.0, 
                        'ks_total_cre': 0.0, 
                        'ks_total_bln': 0.0,
                        'ks_string': ks_df_informations['date'].get('ks_string')
                    }
            }

            initial_start_date = ks_df_informations['date'].get('ks_start_date')
            if ks_df_informations.get('ks_differ')['ks_differentiate_filter'] != 'no_differentiation':
                initial_start_date = ks_df_informations['ks_differ']['ks_start_date']
                interval_lis = []
                for interval_dic in ks_df_informations['ks_differ']['ks_intervals']:
                    subtotal_comparision.update({
                        interval_dic.get('ks_string') : {
                            'ks_total_deb': 0.0, 
                            'ks_total_cre': 0.0, 
                            'ks_total_bln': 0.0}
                        })
                    interval_lis.append(interval_dic.get('ks_string'))

                if interval_lis and len(interval_lis) != len(list(set(interval_lis))):
                    raise ValidationError("Please create a record under the Fiscal Year configuration. Go to Accounting > Configuration > Accounting > Fiscal Year. Please create/edit the Fiscal Year Quarter lines.")
            for ks_account in ks_account_ids:
                no_pop = False
                if (ks_account.internal_group == 'income' or ks_account.internal_group == 'expense'):
                    ks_move_lines[ks_account.code]['balance_carry_fwd'] = False                    

                if ks_df_informations.get('ks_differ')['ks_differentiate_filter'] == 'no_differentiation':
                    if ks_move_lines[ks_account.code]['balance_carry_fwd'] == False:
                        ks_init_blns = {'initial_balance': 0, 'initial_debit': 0, 'initial_credit': 0}
                    else:
                        KS_WHERE_INIT = WHERE + " AND l.date < '%s'" % initial_start_date
                    ...
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations['date']['ks_string']]['initial_balance'] = ks_init_blns['initial_balance']
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations['date']['ks_string']]['debit'] = ks_deb
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations['date']['ks_string']]['credit'] = ks_cre
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations['date']['ks_string']]['balance'] = ks_init_blns['initial_balance'] + ks_bln

                    temp_dic = ks_move_lines[ks_account.code]['comparision'][ks_df_informations['date']['ks_string']]
                    if temp_dic['initial_balance'] != 0 or temp_dic['debit'] != 0 or temp_dic['credit'] != 0 or temp_dic['balance'] != 0:
                        no_pop = True 


                elif ks_df_informations.get('ks_differ')['ks_differentiate_filter'] != 'no_differentiation':
                    ending_balance = 0.0
                    run_query = True

                    for interval_dic in reversed(ks_df_informations['ks_differ']['ks_intervals']):  
                        if ks_move_lines[ks_account.code]['balance_carry_fwd'] == False:
                            initial_balance = 0.0
                        else:
                            if run_query:
                                KS_WHERE_INIT = WHERE + " AND l.date < '%s'" % interval_dic.get('ks_start_date')
                                KS_WHERE_INIT += " AND a.code = %s" % "\'" + ks_account.code + '\''
                                sql = ('''
                                                SELECT
                                                    COALESCE(SUM(l.debit),0) AS initial_debit,
                                ...
                                cr.execute(sql)
                                ks_init_blns = cr.dictfetchone()
                                initial_balance = ks_init_blns['initial_balance']
                                run_query = False
                            else:
                                initial_balance = ending_balance

                        ks_move_lines[ks_account.code]['comparision'].update({
                                interval_dic.get('ks_string') : {'initial_balance': initial_balance, 'debit': 0.0, 'credit': 0.0, 'balance': 0.0, 'ks_string': interval_dic.get('ks_string')}
                            })                            
                        
                        if self.ks_date_filter.get('ks_process') == 'range':
                            KS_WHERE_CURRENT = WHERE + " AND l.date >= '%s'" % interval_dic.get(
                                'ks_start_date') + " AND l.date <= '%s'" % interval_dic.get('ks_end_date')
                        else:
                            KS_WHERE_CURRENT = WHERE + " AND l.date <= '%s'" \
                                               % interval_dic.get('ks_end_date')
                        KS_WHERE_CURRENT += " AND a.code = %s" %  "\'" + ks_account.code + '\''
                        ...
                        ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]['debit'] = ks_deb
                        ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]['credit'] = ks_cre
                        ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]['balance'] = initial_balance + ks_bln
                        ending_balance = ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]['balance']

                        temp_dic = ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]
                        if temp_dic['initial_balance'] != 0 or temp_dic['debit'] != 0 or temp_dic['credit'] != 0 or temp_dic['balance'] != 0:
                            no_pop = True                        
                    ...
                    ks_move_lines[ks_account.code]['comparision'].update({
                        ks_df_informations.get('date')['ks_string'] : {'initial_balance': initial_balance, 'debit': 0.0, 'credit': 0.0, 'balance': 0.0, 'ks_string': ks_df_informations.get('date')['ks_string']}
                    })

                    if self.ks_date_filter.get('ks_process') == 'range':
                        KS_WHERE_CURRENT = WHERE + " AND l.date >= '%s'" % ks_df_informations.get('date').get(
                            'ks_start_date') + " AND l.date <= '%s'" % ks_df_informations.get('date').get('ks_end_date')
                    else:
                        KS_WHERE_CURRENT = WHERE + " AND l.date <= '%s'" \
                                           % ks_df_informations.get('date').get('ks_end_date')
                    KS_WHERE_CURRENT += " AND a.code = %s" %  "\'" + ks_account.code + '\''
                    ...
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations.get('date')['ks_string']]['debit'] = ks_deb
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations.get('date')['ks_string']]['credit'] = ks_cre
                    ks_move_lines[ks_account.code]['comparision'][ks_df_informations.get('date')['ks_string']]['balance'] = initial_balance + ks_bln
                    ending_balance = ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]['balance']

                    temp_dic = ks_move_lines[ks_account.code]['comparision'][ks_df_informations.get('date')['ks_string']]
                    if temp_dic['initial_balance'] != 0 or temp_dic['debit'] != 0 or temp_dic['credit'] != 0 or temp_dic['balance'] != 0:
                        no_pop = True

                if no_pop == False and ks_account.user_type_id.name != 'Current Year Earnings':
                    ks_move_lines.pop(ks_account.code)
                    continue
            
            # print('\nks_move_lines ==',ks_move_lines)
            for account in ks_move_lines.values():
                account["comparision"] = dict(sorted(account["comparision"].items(), key=lambda x: x[0]))

            # print("\nPREEEEEEEE  ks_move_lines ===",ks_move_lines)

            total_balance, total_credit, total_debit = 0.0, 0.0, 0.0
            for key, value in ks_move_lines.items():
                for year, comparison_data in value["comparision"].items():
                    total_balance += comparison_data["balance"]
                    total_credit += comparison_data["credit"]
                    total_debit += comparison_data["debit"]

            for key, value in subtotal_comparision.items():
                value['ks_total_deb'] = ks_company_currency_id.round(value['ks_total_deb'])
                value['ks_total_cre'] = ks_company_currency_id.round(value['ks_total_cre'])
                value['ks_total_bln'] = ks_company_currency_id.round(value['ks_total_bln'])


            ###############  Undistributed Profit/Losses logic  ###############
            new_ks_child_reports = self.env.ref('ks_dynamic_financial_report.ks_df_pnl0')._ks_fetch_children_by_order()

            curr_earnings_acc = self.env['account.account'].search([('company_id','=',ks_df_informations.get('company_id')),
                    ('user_type_id.name','=','Current Year Earnings')], limit=1)

            filter_ctx_dic = ks_df_informations.get('ks_filter_context')

            u_date_from = filter_ctx_dic.get('date_from')
            u_date_to = filter_ctx_dic.get('date_to')

            #Below lines of code added by dhaneswar
            date_from = filter_ctx_dic.get('date_from')
            if date_from:
                end_date = datetime.datetime.strptime(date_from, DEFAULT_SERVER_DATE_FORMAT) - datetime.timedelta(days=1)
                new_date_to = end_date.strftime(DEFAULT_SERVER_DATE_FORMAT)
                ...
                _logger.warning("Missing 'date_from' in filter_ctx_dic, skipping update.")

            pl_res = self.with_context(filter_ctx_dic)._ks_calculate_report_balance(new_ks_child_reports, ks_df_informations)
        
            pl_dic = pl_res.get(self.env.ref('ks_dynamic_financial_report.ks_df_pnl0').id)
            print("\npl_dic ====",pl_dic)

            filter_ctx_dic.update({'date_from': u_date_from, 'date_to': u_date_to})
            ks_df_informations['ks_filter_context'].update({'date_from': u_date_from, 'date_to': u_date_to})

            if pl_dic['balance'] != 0:
                if curr_earnings_acc.code in ks_move_lines:
                    undistributed_line_dic = ks_move_lines[curr_earnings_acc.code]      

                    if pl_dic['balance'] < 0:
                        pl_dic['balance'] = abs(pl_dic['balance'])
                    elif pl_dic['balance'] > 0:
                        pl_dic['balance'] = -1 * pl_dic['balance']

                    new_undis_initial_balance = undistributed_line_dic['initial_balance'] + pl_dic['balance']
                    print("new_undis_initial_balance ===",new_undis_initial_balance)

                    undistributed_line_dic['comparision'][ks_df_informations.get('date')['ks_string']]['initial_balance'] = ks_company_currency_id.round(new_undis_initial_balance)

                    undistributed_line_dic['comparision'][ks_df_informations.get('date')['ks_string']]['balance'] = ks_company_currency_id.round(new_undis_initial_balance + undistributed_line_dic['comparision'][ks_df_informations.get('date')['ks_string']]['debit'] - undistributed_line_dic['comparision'][ks_df_informations.get('date')['ks_string']]['credit'])
                    
                    ks_move_lines[curr_earnings_acc.code] = undistributed_line_dic
            ...
            if ks_df_informations.get('ks_differ')['ks_differentiate_filter'] != 'no_differentiation':
                ...
                for interval_dic in reversed(ks_df_informations['ks_differ']['ks_intervals']):  
                    ...
                    #Below lines of code added by dhaneswar
                    date_from = interval_dic['ks_start_date']
                    if date_from:
                        end_date = datetime.datetime.strptime(date_from, DEFAULT_SERVER_DATE_FORMAT) - datetime.timedelta(days=1)
                        new_date_to = end_date.strftime(DEFAULT_SERVER_DATE_FORMAT)
                    ...
                    if pl_dic['balance'] != 0:
                        if curr_earnings_acc.code in ks_move_lines:
                            ...
                            new_undis_initial_balance = undistributed_line_dic['comparision'][interval_dic['ks_string']]['initial_balance'] + pl_dic['balance']
                            print("new_undis_initial_balance ===",new_undis_initial_balance)

                            undistributed_line_dic['comparision'][interval_dic['ks_string']]['initial_balance'] = ks_company_currency_id.round(new_undis_initial_balance)

                            undistributed_line_dic['comparision'][interval_dic['ks_string']]['balance'] = ks_company_currency_id.round(new_undis_initial_balance + undistributed_line_dic['comparision'][interval_dic['ks_string']]['debit'] - undistributed_line_dic['comparision'][interval_dic['ks_string']]['credit'])
                            
                            ks_move_lines[curr_earnings_acc.code] = undistributed_line_dic
                        
            ks_move_lines = dict(sorted(ks_move_lines.items()))
            ...
            # Get all years across all accounts
            years = set()
            for account in ks_move_lines.values():
                years.update(account["comparision"].keys())

            # Prepare default year-wise subtotal entries
            for year in years:
                ks_subtotal["SUBTOTAL"]["subtotal_comparision"][year] = {
                    "ks_total_initial_bln": 0.0,
                    "ks_total_deb": 0.0,
                    "ks_total_cre": 0.0,
                    "ks_total_bln": 0.0,
                    "ks_string": year,
            ...
            # Sum up values year-wise
            for account in ks_move_lines.values():
                for year, values in account["comparision"].items():
                    subtotal_entry = ks_subtotal["SUBTOTAL"]["subtotal_comparision"][year]
                    subtotal_entry["ks_total_initial_bln"] += values.get("initial_balance", 0.0)
                    subtotal_entry["ks_total_deb"] += values.get("debit", 0.0)
                    subtotal_entry["ks_total_cre"] += values.get("credit", 0.0)
                    subtotal_entry["ks_total_bln"] += values.get("balance", 0.0)

            # Optional: Sort by year
            ks_subtotal["SUBTOTAL"]["subtotal_comparision"] = dict(
                sorted(ks_subtotal["SUBTOTAL"]["subtotal_comparision"].items(), key=lambda x: x[0])
            )

            for year_data in ks_subtotal['SUBTOTAL']['subtotal_comparision'].values():
                for key in year_data:
                    if isinstance(year_data[key], float):
                        year_data[key] = round(year_data[key], 2)  # Adjust decimal places as needed

            print("\nFINALLLLLLLLLLL  ks_move_lines ===",ks_move_lines)
            print("\nFINALLLLLLLLLLL  ks_subtotal ===",ks_subtotal)
            return ks_move_lines, {}, ks_subtotal
    ...
    def ks_process_trial_balance(self, ks_df_informations):
        ...
        if ks_df_informations:
            ...
            # ks_account_ids = ks_account_ids.filtered(lambda x: x.id == 14495)
            ks_move_lines = {x.code: {'name': x.name, 'code': x.code, 'id': x.id, 'account_user_type': x.user_type_id.name,
                                      'initial_debit': 0.0, 'initial_credit': 0.0, 'initial_balance': 0.0,
                                      'debit': 0.0, 'credit': 0.0, 'balance': 0.0,
                                      'ending_credit': 0.0, 'ending_debit': 0.0, 'ending_balance': 0.0,
                                      'company_currency_id': ks_company_currency_id.id,
            ...
            if ks_df_informations.get('ks_differ')['ks_differentiate_filter'] == 'no_differentiation':
                no_pop = False
                account_codes = [ks_account.code for ks_account in ks_account_ids]

                sql = ('''
                    SELECT
                        a.code AS account_code,
                        a.internal_group AS account_internal_group,
                        t.name AS user_type_name,
                        COALESCE(SUM(CASE WHEN l.date < '%(initial_start_date)s' THEN l.debit END),0) -
                        COALESCE(SUM(CASE WHEN l.date < '%(initial_start_date)s' THEN l.credit END),0) AS initial_balance,
                        COALESCE(SUM(CASE WHEN l.date >= '%(ks_start_date)s' AND l.date <= '%(ks_end_date)s' THEN l.debit END),0) AS debit,
                        COALESCE(SUM(CASE WHEN l.date >= '%(ks_start_date)s' AND l.date <= '%(ks_end_date)s' THEN l.credit END),0) AS credit,
                        COALESCE(SUM(CASE WHEN l.date >= '%(ks_start_date)s' AND l.date <= '%(ks_end_date)s' THEN l.debit END),0) -
                        COALESCE(SUM(CASE WHEN l.date >= '%(ks_start_date)s' AND l.date <= '%(ks_end_date)s' THEN l.credit END),0) AS balance
                    FROM account_move_line l
                    JOIN account_move m ON (l.move_id = m.id)
                    JOIN account_account a ON (l.account_id = a.id)
                    JOIN account_account_type t ON (a.user_type_id = t.id)
                    LEFT JOIN account_analytic_account anl ON (l.analytic_account_id = anl.id)
                    LEFT JOIN res_currency c ON (l.currency_id = c.id)
                    LEFT JOIN res_partner p ON (l.partner_id = p.id)
                    JOIN account_journal j ON (l.journal_id = j.id)
                    WHERE %(WHERE)s
                      AND a.code IN (%(account_codes)s)
                    GROUP BY a.code, t.name, a.internal_group
                ''') % {
                    'initial_start_date': initial_start_date,
                    'ks_start_date': ks_df_informations['date'].get('ks_start_date'),
                    'ks_end_date': ks_df_informations['date'].get('ks_end_date'),
                    'WHERE': WHERE,
                    'account_codes': ",".join("'%s'" % c for c in account_codes)
                }
                ...
                for row in results: 
                    ...
                    valid_codes.append(acc_code)

                    if (row['account_internal_group'] == 'income' or row['account_internal_group'] == 'expense'):
                        ks_move_lines[acc_code]['balance_carry_fwd'] = False

                    if ks_move_lines[acc_code]['balance_carry_fwd'] == False:
                        initial_balance = 0
                    else:
                    ...
                    ks_move_lines[acc_code]['comparision'][ks_df_informations['date']['ks_string']]['initial_balance'] = initial_balance
                    ks_move_lines[acc_code]['comparision'][ks_df_informations['date']['ks_string']]['debit'] = ks_deb
                    ks_move_lines[acc_code]['comparision'][ks_df_informations['date']['ks_string']]['credit'] = ks_cre
                    ks_move_lines[acc_code]['comparision'][ks_df_informations['date']['ks_string']]['balance'] = initial_balance + ks_bln

                    temp_dic = ks_move_lines[acc_code]['comparision'][ks_df_informations['date']['ks_string']]
                    if temp_dic['initial_balance'] != 0 or temp_dic['debit'] != 0 or temp_dic['credit'] != 0 or temp_dic['balance'] != 0:
                        no_pop = True 

                    if no_pop == False and row['user_type_name'] != 'Current Year Earnings':
                        ks_move_lines.pop(acc_code)
                        continue

                # Remove keys not in valid_codes
                ks_move_lines = {k: v for k, v in ks_move_lines.items() if k in valid_codes or v['account_user_type'] == 'Current Year Earnings'}

            else:
                ks_account_pop = {}
                for ks_account in ks_account_ids:
                    ks_account_pop.update({ks_account.code: False})
                    if (ks_account.internal_group == 'income' or ks_account.internal_group == 'expense'):
                        ks_move_lines[ks_account.code]['balance_carry_fwd'] = False                    
                    ...
                    for interval_dic in reversed(ks_df_informations['ks_differ']['ks_intervals']):  
                        ...
                        temp_dic = ks_move_lines[ks_account.code]['comparision'][interval_dic['ks_string']]
                        if temp_dic['initial_balance'] != 0 or temp_dic['debit'] != 0 or temp_dic['credit'] != 0 or temp_dic['balance'] != 0:
                            ks_account_pop[ks_account.code] = True
                ...
                for row in results:                                       
                    acc_code = row['account_code']
                    no_pop = ks_account_pop[acc_code]
    ...
    def ks_process_tax_report(self, ks_df_informations):
        if ks_df_informations:
            ...
            # Metro Custom tax report pdf Code: Inject AML detail lines when checkbox is enabled
            if ks_df_informations.get("ks_report_with_lines"):
                grouped_detail_lines = []

                for line in ks_tax_line:
                    tax_id = line.get("id")
                    if tax_id is None:
                        continue
                    ...
                    # Domain: always fetch base lines with this tax
                    domain = [
                        ("tax_ids", "=", tax_id),
                    ...
                    if date_vals.get("ks_start_date"):
                        domain.append(("date", ">=", date_vals["ks_start_date"]))
                    if date_vals.get("ks_end_date"):
                        domain.append(("date", "<=", date_vals["ks_end_date"]))

                    aml_records = self.env["account.move.line"].search(domain, order="date, id")

                    # Logic to fetch proper label from invoice product lines
                    # aml_dict = {}
                    ...
                    #                   and (not l.name or not l.name.startswith("INV"))
                    #     )
                    #     names = [l.name.strip() for l in main_lines if l.name and l.name.strip()]
                    #     concat_names = ", ".join(names) if names else ""
                    #     tax_lines = move_lines.filtered(lambda l: l.tax_line_id)
                    #     for tax_line in tax_lines:
                    #         aml_dict[tax_line.id] = concat_names

                    grouped_by_invoice = {}
                    for move in aml_records.mapped("move_id"):
                        key = (move.id, tax.id)

                        # ✅ Tax amount: only from tax lines
                        tax_amt = sum(
                            abs(l.credit if tax.type_tax_use == "sale" else l.debit)
                            for l in move.line_ids if l.tax_line_id == tax
                        )

                        # ✅ Base amount: only from base lines linked with this tax
                        base_lines = move.line_ids.filtered(lambda l: tax in l.tax_ids and not l.tax_line_id)
                        base_amt = sum(
                            abs(l.credit if tax.type_tax_use == "sale" else l.debit)
                            for l in base_lines
                        )

                        if key not in grouped_by_invoice:
                            grouped_by_invoice[key] = {
                                "date": str(move.date),
                                "partner_name": move.partner_id.name or "",
                                "account": base_lines[:1].account_id.display_name or "",
                                "inv_ref": move.name or "",
                                "label": move.ref or "",
                                "tax_rate": ("%g%%" % tax.amount),
                                "currency": move.currency_id.name or ks_company_currency_id.name,
                                "base_amt": 0.0,
                                "tax_amt": 0.0,
                            }

                        grouped_by_invoice[key]["base_amt"] += base_amt
                        grouped_by_invoice[key]["tax_amt"] += tax_amt

                    tax_details = []
                    subtotal_total_amt = 0.0
                    subtotal_base_amt = 0.0
                    subtotal_tax_amt = 0.0

                    for vals in grouped_by_invoice.values():
                        total_amt = vals["base_amt"] + vals["tax_amt"]
                        vals["total_amt"] = total_amt

                        tax_details.append(vals)

                        subtotal_total_amt += total_amt
                        subtotal_base_amt += vals["base_amt"]
                        subtotal_tax_amt += vals["tax_amt"]

                    if tax_details or tax.amount == 0.0:
                        grouped_detail_lines.append({
                            "tax_name": tax.name,
                            "tax_rate": "%g%%" % tax.amount,
                            "lines": tax_details,
                            "subtotal_total_amt": subtotal_total_amt,
                            "subtotal_base_amt": subtotal_base_amt,
                            "subtotal_tax_amt": subtotal_tax_amt,
                        })

                ks_df_informations["ks_tax_detail_lines"] = grouped_detail_lines
                ks_df_informations["display_customer_name_on_tax_report"] = ks_company_id.display_customer_name_on_tax_report
    ...
    def ks_build_detailed_move_lines(self, offset=0, partner=0, ks_df_informations=False, partner_ledger=False, fetch_range=FETCH_RANGE):
        ...
        if ((count - ks_offset_count) <= fetch_range) and ks_df_informations.get('initial_balance'):
            # print('KS_WHERE_FULL ==',KS_WHERE_FULL)
            sql = ('''
                    SELECT
                        COALESCE(SUM(l.debit),0) AS debit,
                        COALESCE(SUM(l.credit),0) AS credit,
    ...
    def ks_partner_aging_process_data(self, ks_df_informations):
        ...
        ctx = self.env.context.copy()
        ctx.update({'ks_aging_by_month_option': ks_df_informations.get('ks_aging_by_month', False)})
        ks_period_dict = self.with_context(ctx).ks_prepare_due_bucket_list(ks_as_on_date)
        ks_company_id = self.env['res.company'].sudo().browse(ks_df_informations.get('company_id'))
        ...
        ks_partner_aging_data = self.ks_process_aging_data(ks_df_informations, offset=0,ks_partner=ks_partner_ids.ids,
                                                           fetch_range=FETCH_RANGE)[4]
        ...
        for ks_partner in ks_partner_ids:
            ...
            sql = """
                ...
                WHERE
                    ...
                    AND l.partner_id = %s
                    AND l.date <= '%s'
                    AND l.company_id in %s AND m.adjust_in_carry_fwd_bal IS NOT TRUE
            ...
            if count:
                ...
                ks_partner_dict[ks_partner.id]['lines'] = ks_partner_aging_data and ks_partner_aging_data.get(ks_partner.id, [])

                # Calculate the total balance based on the filtered lines
                ks_total_balance = sum(line['balance'] for line in ks_partner_dict[ks_partner.id]['lines'])
                # print("\n\n ks_total_balance >  >>>>>>>>>>>> ======",ks_total_balance)

                # ks_partner_dict[ks_partner.id].update({'total': ks_total_balance})

                dic = {'Not Due': 0.0, '1 - 30': 0.0, '31 - 60': 0.0, '61 - 90': 0.0, '91 - 120': 0.0, '121 - 180': 0.0,
                       '180 +': 0.0}
                dic_month = {'Not Due': 0.0, '1 Month': 0.0, '2 Month': 0.0, '3 Month': 0.0, '4 Month': 0.0,
                             '5 Month': 0.0,
                             '6 Month+': 0.0}
                # main_dic2  = {'range_0': 'Not Due', 'range_1': '1-30', 'range_2':'31 - 60' ..............}

                check_aging_by_month = ks_df_informations and ks_df_informations.get('ks_aging_by_month')
                if not check_aging_by_month:
                    for each_line in ks_partner_dict[ks_partner.id]['lines']:
                        dic['Not Due'] += each_line['range_0']
                        dic['1 - 30'] += each_line['range_1']
                        dic['31 - 60'] += each_line['range_2']
                        dic['61 - 90'] += each_line['range_3']
                        dic['91 - 120'] += each_line['range_4']
                        dic['121 - 180'] += each_line['range_5']
                        dic['180 +'] += each_line['range_6']
                        # for key, value in dic.items():
                        #     print(f"\n{key}: {value}", end=" ")

                    ks_partner_dict['Total']['total'] += ks_total_balance
                    ks_partner_dict['Total']['Not Due'] += dic['Not Due']
                    ks_partner_dict['Total']['1 - 30'] += dic['1 - 30']
                    ks_partner_dict['Total']['31 - 60'] += dic['31 - 60']
                    ks_partner_dict['Total']['61 - 90'] += dic['61 - 90']
                    ks_partner_dict['Total']['91 - 120'] += dic['91 - 120']
                    ks_partner_dict['Total']['121 - 180'] += dic['121 - 180']
                    ks_partner_dict['Total']['180 +'] += dic['180 +']

                    ks_partner_dict[ks_partner.id].update(
                        {'Not Due': dic['Not Due'], '1 - 30': dic['1 - 30'], '31 - 60': dic['31 - 60'],
                         '61 - 90': dic['61 - 90'], '91 - 120': dic['91 - 120'], '121 - 180': dic['121 - 180'],
                         '180 +': dic['180 +'], 'total': ks_total_balance})

                elif check_aging_by_month:
                    for each_line in ks_partner_dict[ks_partner.id]['lines']:
                        dic_month['Not Due'] += each_line['range_0']
                        dic_month['1 Month'] += each_line['range_1']
                        dic_month['2 Month'] += each_line['range_2']
                        dic_month['3 Month'] += each_line['range_3']
                        dic_month['4 Month'] += each_line['range_4']
                        dic_month['5 Month'] += each_line['range_5']
                        dic_month['6 Month+'] += each_line['range_6']
                        # for key, value in dic_month.items():
                        #     print(f"\n{key}: {value}", end=" ")

                    ks_partner_dict['Total']['total'] += ks_total_balance
                    ks_partner_dict['Total']['Not Due'] += dic_month['Not Due']
                    ks_partner_dict['Total']['1 Month'] += dic_month['1 Month']
                    ks_partner_dict['Total']['2 Month'] += dic_month['2 Month']
                    ks_partner_dict['Total']['3 Month'] += dic_month['3 Month']
                    ks_partner_dict['Total']['4 Month'] += dic_month['4 Month']
                    ks_partner_dict['Total']['5 Month'] += dic_month['5 Month']
                    ks_partner_dict['Total']['6 Month+'] += dic_month['6 Month+']

                    ks_partner_dict[ks_partner.id].update(
                        {'Not Due': dic_month['Not Due'], '1 Month': dic_month['1 Month'],
                         '2 Month': dic_month['2 Month'],
                         '3 Month': dic_month['3 Month'], '4 Month': dic_month['4 Month'],
                         '5 Month': dic_month['5 Month'],
                         '6 Month+': dic_month['6 Month+'], 'total': ks_total_balance})

                ks_partner_dict[ks_partner.id].update({'company_currency_id': company_currency_id})
                ks_partner_dict['Total'].update({'company_currency_id': company_currency_id})
                if ks_partner_dict[ks_partner.id]['total'] == 0.0:
                    ks_partner_dict.pop(ks_partner.id, None)

                # print("\n\nks_partner_dict ========",ks_partner_dict)
            else:
                ks_partner_dict.pop(ks_partner.id, None)
        # print("\nks_period_dict ====",ks_period_dict)
    ...
    def ks_process_aging_data(self, ks_df_informations, offset=0, ks_partner=0, fetch_range=FETCH_RANGE):
        ...
        :return: count(int-Total rows without offset), offset(integer), ks_move_lines(list of dict)
        '''
        # print("\n\n>>>>>>>>>>>>>>>>>>>>>>>>.  ks_process_aging_data()")
        # print("ks_partner ====",ks_partner)
        ...
        if isinstance(ks_partner, list) and len(ks_partner) > 1:
            ks_partner_sql = f"l.partner_id IN {tuple(ks_partner)}"
        else:
            ks_partner = ks_partner[0] if isinstance(ks_partner, list) else ks_partner
            ks_partner_sql = f"l.partner_id = {ks_partner}"
        ...
        if ks_partner:
            sql = """
                    ...
                    WHERE
                        ...
                        AND l.date <= '%s'
                        AND l.company_id in %s AND m.adjust_in_carry_fwd_bal IS NOT TRUE
                """ % (WHERE, ks_type, ks_partner_sql, ks_as_on_date, str(tuple(ks_company_ids) + tuple([0])))
            self.env.cr.execute(sql)
            count = self.env.cr.fetchone()[0]

            SELECT = """SELECT m.name AS move_name, l.partner_id AS partner_id,
                                m.id AS move_id,
                                l.date AS date,
            ...
            payment_type = 'inbound' if ks_type == 'receivable' else 'outbound'
            balance_operator = '<' if ks_type == 'receivable' else '>'
            if ks_df_informations.get('ks_posted_entries') and not ks_df_informations.get('ks_unposted_entries'):
                move_state = "am.state = 'posted' "
            elif ks_df_informations.get('ks_unposted_entries') and not ks_df_informations.get('ks_posted_entries'):
                move_state = "am.state = 'draft' "
            else:
                move_state = "am.state in  ('posted', 'draft')"
                ...
                if ks_period_dict[ks_period].get('start') and ks_period_dict[ks_period].get('stop'):
                    SELECT += """ CASE 
                                    ...
                                    THEN
                                        ...
                                        COALESCE((CASE WHEN j.type = 'general' THEN 
                                               (SELECT SUM(aml.balance)
                                                FROM account_move_line aml
                                                LEFT JOIN account_move am ON am.id = aml.move_id
                                                LEFT JOIN account_payment ap ON ap.id = am.payment_id
                                                WHERE aml.partner_id = l.partner_id
                                                  AND am.adjust_in_carry_fwd_bal IS TRUE
                                                  AND aml.balance %s 0 
                                                  AND aml.date <= '%s'
                                                  AND ap.payment_type = '%s'
                                                  AND aml.company_id = l.company_id
                                                  AND %s)
                                                ELSE 0 END), 0)
                                    ELSE
                                        0
                    ...
                    SELECT += """ CASE
                                    ...
                                    ELSE
                                        0
                                    END AS %s """ % (ks_period_dict[ks_period].get('start'),
                                                     ks_as_on_date,
                                                     ks_as_on_date,
                                                     balance_operator,
                                                     ks_as_on_date,
            ...
            sql = """
                    ...
                    WHERE
                        ...
                        AND l.company_id in %s AND m.adjust_in_carry_fwd_bal IS NOT TRUE
                    GROUP BY
                        l.partner_id,l.date, l.date_maturity, m.id, m.name, j.name, a.name, cc.id, j.type, l.company_id
                    OFFSET %s ROWS
                """ % (
            WHERE, ks_type, ks_partner_sql, ks_as_on_date, str(tuple(ks_company_ids) + tuple([0])), offset)
            ...
            for ks_fn_lst in ks_final_list:
                if (ks_fn_lst['range_0'] or ks_fn_lst['range_1'] or ks_fn_lst['range_2'] or ks_fn_lst['range_3'] or
                        ks_fn_lst['range_4'] or ks_fn_lst['range_5'] or ks_fn_lst['range_6']):
                    ks_move_lines.append(ks_fn_lst)

            grouped_move_lines = defaultdict(list)
            ks_move_lines_new = []
            for ks_fn_lst in ks_move_lines:
                total = sum(ks_fn_lst[f'range_{i}'] for i in range(len(ks_period_dict)))
                ks_fn_lst['balance'] = ks_currency.round(total)
                if (ks_fn_lst['range_0'] or ks_fn_lst['range_1'] or ks_fn_lst['range_2'] or ks_fn_lst['range_3'] or
                    ks_fn_lst['range_4'] or ks_fn_lst['range_5'] or ks_fn_lst['range_6']) and not (
                        'e' in str(ks_fn_lst['range_0']) or 'e' in str(ks_fn_lst['range_1']) or 'e' in str(
                    ks_fn_lst['range_2']) or 'e' in str(ks_fn_lst['range_3']) or 'e' in str(
                    ks_fn_lst['range_4']) or 'e' in str(ks_fn_lst['range_5']) or 'e' in str(ks_fn_lst['range_6'])):
                    partner_id = ks_fn_lst['partner_id']
                    grouped_move_lines[partner_id].append(ks_fn_lst)
                    ks_move_lines_new.append(ks_fn_lst)

            ks_move_lines_grouped = dict(grouped_move_lines)
            if ks_move_lines_grouped:
                return count, offset, ks_move_lines_new, ks_period_list, ks_move_lines_grouped
            else:
                return 0, 0, [], [], []

    def ks_prepare_due_bucket_list(self, ks_as_on_date =False):
        """ Metro Customizations inside """
        ks_periods = {}
        ...
        ctx = self.env.context.copy()
        check_aging_by_month = ctx and ctx.get('ks_aging_by_month_option')
        ...
        if not check_aging_by_month:
            ...
            ks_final_date = False
            for i in range(5):
                ks_stop = ks_start.replace(day=1) - datetime.timedelta(days=1)
                ks_start = ks_stop.replace(day=1)
                ks_bucket_name = str(i+1) + ' ' + 'Month'
                ks_final_date = ks_start
                ks_periods[i + 1] = {
                    'ks_due_bucket': ks_due_bucket_list[i],
                    'name': ks_bucket_name,
    ...
    def post_process_balance_sheet_ks_report_lines(self, ks_report_lines):
        # _logger.info('\n\n\n\n>>>>>>>>>>>>>  post_process_balance_sheet_ks_report_lines() >>>>>>')
        # _logger.info(self)
        # _logger.info(ks_report_lines)

        for dic in [line_dic for line_dic in ks_report_lines if line_dic['account_type'] == 'total']:
            report_obj = self.env['ks.dynamic.financial.reports'].browse(dic['self_id'])
            if report_obj.ks_parent_id.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet').id and \
                        report_obj.ks_df_report_account_type == 'total' and report_obj.ks_custom_report_1 and report_obj.ks_custom_report_2 or \
                        (report_obj.ks_custom_report_1.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_liability0').id and \
                        report_obj.ks_custom_report_2.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_equity').id) or \
                        (report_obj.ks_custom_report_1.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_equity').id and \
                        report_obj.ks_custom_report_2.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_liability0').id):
                equity_balance, liability_balance = 0.0, 0.0
                new_balance_cmp = {key: 0 for key in dic['balance_cmp']}
                for d in ks_report_lines:
                    if d['self_id'] == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_equity').id:
                        equity_balance = d['balance']
                        for key, value in d['balance_cmp'].items():
                            new_balance_cmp[key] += value

                    elif d['self_id'] == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_liability0').id:
                        liability_balance = d['balance']
                        for key, value in d['balance_cmp'].items():
                            new_balance_cmp[key] += value

                dic['balance'] = equity_balance + liability_balance
                if new_balance_cmp:
                    dic['balance_cmp'] = new_balance_cmp

        if not self.env.company.merge_bank_accounts:
            return ks_report_lines

        # merge logic begins
        # ---------------------------------------------------------
        # Merge balances of mapped accounts (child → parent)
        # result = {parent_account_id: [child_account_ids]}
        # ---------------------------------------------------------


        result = {}
        for journal in self.env['account.journal'].search([('company_id','=',self.env.company.id),('type', 'in', ['bank','cash'])]):
            if journal.default_account_id:
                result[journal.default_account_id.id] = [
                    journal.payment_debit_account_id.id
                        if journal.payment_debit_account_id else False,
                    journal.payment_credit_account_id.id
                        if journal.payment_credit_account_id else False
                ]
        # _logger.info('\n\nresult as dictionary:')
        # _logger.info(result) # {167383: [167384, 167385], 74477: [74477, 74477], 74476: [82206, 82207], 74479: [82204, 82205], 74475: [82208, 82209], 71219: [71220, 71221], 86213: [86214, 86215], 85880: [85881, 85882], 85877: [85878, 85879], 71216: [71217, 71218]}

        # Build quick lookup: account_id -> line dict
        account_line_map = {
            line['account']: line for line in ks_report_lines if line.get('account')
        }

        # Track child lines to remove
        lines_to_remove = set()

        for parent_acc, child_acc_list in result.items():
            parent_line = account_line_map.get(parent_acc)
            if not parent_line:
                continue

            for child_acc in filter(None, child_acc_list):
                child_line = account_line_map.get(child_acc)
                if not child_line or child_line == parent_line:
                    continue

                # 1️⃣ Merge balances
                parent_line['balance'] += child_line.get('balance', 0.0)

                # 2️⃣ Merge debit / credit if present
                parent_line['debit'] = parent_line.get('debit', 0.0) + child_line.get('debit', 0.0)
                parent_line['credit'] = parent_line.get('credit', 0.0) + child_line.get('credit', 0.0)

                # 3️⃣ Merge comparison balances
                if parent_line.get('balance_cmp') and child_line.get('balance_cmp'):
                    for k, v in child_line['balance_cmp'].items():
                        parent_line['balance_cmp'][k] = parent_line['balance_cmp'].get(k, 0) + v

                # Mark child line for removal
                # if not child_line == parent_line:#
                lines_to_remove.add(id(child_line))

        # 4️⃣ Remove child account dictionaries
        ks_report_lines = [
            line for line in ks_report_lines if id(line) not in lines_to_remove
        ]

        # _logger.info("\n\n 22222222222  ks_report_lines")
        # _logger.info(ks_report_lines)

        # ---------------------------------------------------------
        # Re-sum balances for affected parent report lines (86, 88)
        # because child account lines were removed
        # ---------------------------------------------------------

        other_current_assets_id = next(
            (
                line.get('self_id')
                for line in ks_report_lines
                if line.get('ks_name') == 'Other Current Assets'
                and line.get('ks_df_report_account_type') == 'report'
            ),
            None
        )
        RESUM_PARENT_IDS = {self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet_bank_cash').id, other_current_assets_id}

        # _logger.info(RESUM_PARENT_IDS)
        
        # Group lines by parent self_id
        children_by_parent = {}
        for line in ks_report_lines:
        ...
        for line in ks_report_lines:
            if line.get('self_id') in RESUM_PARENT_IDS:
                total_balance = 0.0
                total_debit = 0.0
                total_credit = 0.0
                new_balance_cmp = {}

                for child in children_by_parent.get(line['self_id'], []):
                    total_balance += child.get('balance', 0.0)
                    total_debit += child.get('debit', 0.0)
                    total_credit += child.get('credit', 0.0)

                    if child.get('balance_cmp'):
                        for k, v in child['balance_cmp'].items():
                            new_balance_cmp[k] = new_balance_cmp.get(k, 0) + v
                ...
                if new_balance_cmp:
                    line['balance_cmp'] = new_balance_cmp
    ...
    def ks_get_dynamic_fin_info(self, ks_df_informations,offset={}):
        print("\n /////////////////////////////\n\n\nks_get_dynamic_fin_info() >>>>>>>>",self, self._context)
        _logger.info("\n\n\n......")
        ctx = self._context or {}

        show_print_pdf_btn = True
        show_print_xls_btn = True

        set_default = False
        if not ks_df_informations:
            set_default = True

        ks_df_informations = self._ks_get_df_informations(ks_df_informations)
        _logger.info(ks_df_informations)
        if 'external_call_bs' in ctx:
            ks_df_informations['date'].update(ctx['external_call_bs']['date'])
        elif 'external_call_pl' in ctx:
            ks_df_informations['date'] = ctx['external_call_pl']['date']
            ks_df_informations['ks_differ'] = ctx['external_call_pl']['ks_differ']
        elif 'external_call_cf' in ctx:
            ks_df_informations['date'].update(ctx['external_call_cf']['date'])
        elif 'external_call_tax' in ctx and ('ks_option_enable' not in ctx or 'ks_journal_enable' not in ctx or 'ks_account_enable' not in ctx):
            ks_df_informations['date'] = ctx['external_call_tax']['date']
            ks_df_informations['ks_posted_entries'] = ctx['external_call_tax']['ks_posted_entries']
        offset_dict = {}

        if set_default:
            if self.id in [self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet').id, self.env.ref('ks_dynamic_financial_report.ks_df_pnl0').id, self.env.ref('ks_dynamic_financial_report.ks_df_gl0').id]:
                ks_df_informations['ks_posted_entries'] = True
            # elif self.id  == self.env.ref('ks_dynamic_financial_report.ks_df_tb0').id:
            #     ks_df_informations['ks_comparison_range'] = True
        ...
        if self.id == self.env.ref('ks_dynamic_financial_report.ks_df_tb0').id:
            ...
            show_print_pdf_btn = False
            show_print_xls_btn = False
        ...
        elif self.id == self.env.ref('ks_dynamic_financial_report.ks_df_report_cash_flow0').id:
            ks_report_lines, ks_initial_balance, ks_current_balance, ks_ending_balance = self.ks_fetch_report_account_lines(
                ks_df_informations)
            ks_report_lines_new = []
            for dic in ks_report_lines:
                bal = dic.get('balance', 0.0)
                if bal < 0.0:
                    dic.update({'balance': -1 * bal})
                ks_report_lines_new.append(dic)
            ks_report_lines = ks_report_lines_new
        else:
            # Updating pagingation in general ledger and partner ledger
            offset_dict = self.ks_calulate_offset(offset)

            ks_report_lines, ks_initial_balance, ks_current_balance, ks_ending_balance = self.ks_fetch_report_account_lines(
                ks_df_informations,offset_dict)

            if self.id == self.env.ref('ks_dynamic_financial_report.ks_dynamic_financial_balancesheet').id and ks_report_lines:
                ks_report_lines = self.post_process_balance_sheet_ks_report_lines(ks_report_lines)
        ...
        offset_dict.update({'show_print_pdf_btn': show_print_pdf_btn, 'show_print_xls_btn': show_print_xls_btn}) #metro
        info = {
            ...
            'ks_buttons': self.env['ir.ui.view']._render_template('ks_dynamic_financial_report.ks_repport_buttons',values=offset_dict),
    ...
    def _ks_get_df_informations(self, ks_earlier_informations=None):
        ks_df_informations = {
            ...
            'ks_aging_by_month': ks_earlier_informations and ks_earlier_informations.get(
                'ks_aging_by_month') or False, # Metro
    ...
    def ks_construct_date_filter(self, ks_df_informations, ks_eariler_informations=None):
        _logger.info("\nks_construct_date_filter() >>>>>")
        if self.ks_date_filter is None:
            return
        res = self.ks_get_default_informations(ks_df_informations, ks_eariler_informations)
        _logger.info("\nFINALLY .... ks_construct_date_filter() >>>>")
        _logger.info(res)
        return res

    def ks_get_default_informations(self, ks_df_informations, ks_earlier_informations):
        _logger.info("\nks_get_default_informations() >>>>>")
        ks_earlier_date = (ks_earlier_informations or {}).get('date', {})
        _logger.info("ks_earlier_date ==")
        _logger.info(ks_earlier_date)
        ...
        # ks_filters_informations = 'this_year'
    ...
    def get_fiscal_quarter_start_end_dates(self, ks_df_informations, ks_filters_informations, current_date):
        """METRO CUSTOM METHOD"""
        if 'quarter' in ks_filters_informations:
            line_obj = self.env['account.fiscal.year.lines'].sudo().search([('parent_id.company_id','=',ks_df_informations.get('company_id')),
                ('date_from','<=',current_date.strftime('%Y-%m-%d')),('date_to','>=',current_date.strftime('%Y-%m-%d'))], limit=1)
            if not line_obj:
                raise ValidationError("The date: " + current_date.strftime('%d/%m/%Y') +" is not found in any of the dates range the Fiscal Year configuration. Go to Accounting > Configuration > Accounting > Fiscal Year. Please create/edit the Fiscal Year Quarter lines.")

            return line_obj.date_from, line_obj.date_to
    ...
    def ks_create_company_date(self, ks_df_informations, ks_eariler_informations, ks_process, ks_filters_informations,
        ...
        if 'month' in ks_filters_informations or 'quarter' in ks_filters_informations or 'year' in ks_filters_informations: #DONE
            if 'month' in ks_filters_informations:
                ...
                # ks_start_date, ks_end_date = date_utils.get_quarter(fields.Date.context_today(self)) METRO commented
                ks_start_date, ks_end_date = self.get_fiscal_quarter_start_end_dates(ks_df_informations, ks_filters_informations, fields.Date.context_today(self))
        ...
        if 'this_year_half' in ks_filters_informations: #DONE
            company_fiscalyear_dates = self.env.company.compute_fiscalyear_dates(fields.Date.context_today(self))
            ks_start_date = company_fiscalyear_dates['date_from']
            ks_end_date = ks_start_date + relativedelta(months=6) + relativedelta(days=-1)
            today = datetime.date.today()
            if today > ks_end_date:
                ks_start_date = company_fiscalyear_dates['date_from'] + relativedelta(months=6)
                ks_end_date = company_fiscalyear_dates['date_to']
        elif 'year_to_date' in ks_filters_informations: #DONE
            ks_start_date = company_fiscalyear_dates['date_from']
            ks_end_date = datetime.date.today()
        elif 'end_last_month' in ks_filters_informations: #DONE
            today = datetime.date.today()
            ...
            ks_start_date = first_day_last_month
            ks_end_date = last_day_last_month
    ...
    @api.model
    def get_fiscal_year_line(self, ks_df_informations, ks_date):
        """METRO CUSTOM METHOD"""
        line_obj = self.env['account.fiscal.year.lines'].sudo().search([('parent_id.company_id','=',ks_df_informations.get('company_id')),
            ('date_from','<=',ks_date.strftime('%Y-%m-%d')),('date_to','>=',ks_date.strftime('%Y-%m-%d'))], limit=1)
        if not line_obj:
            raise ValidationError("The date: " + ks_date.strftime('%d/%m/%Y') + " is not found in the Fiscal Year configuration. Go to Accounting > Configuration > Accounting > Fiscal Year. Please create/edit the Fiscal Year Quarter lines.")
        return line_obj
    ...
    def _ks_construct_date_string(self, ks_df_informations, ks_process, ks_interval_type, ks_end_date, ks_start_date,
        ...
        if not ks_string:
            ...
            if ks_process == 'single':
                ...
                ks_end_date)):
                # ks_string = ks_end_date.strftime('%Y') #METRO COMMENTED
                line_obj = self.get_fiscal_year_line(ks_df_informations, ks_end_date)
                ks_string = line_obj.parent_id.name
                ...
                #     quarter_names[date_utils.get_quarter_number(ks_end_date)], ks_end_date.year) #METRO COMMENTED
                line_obj = self.get_fiscal_year_line(ks_df_informations, ks_end_date)
                quarter_number, quarter_fiscal_year = line_obj.quarter, line_obj.parent_id.name
                ks_string = u'%s\N{NO-BREAK SPACE}%s' % (
                    quarter_number, quarter_fiscal_year)
            else:
    ...
    def ks_fetch_eariler_dates_interval(self, ks_df_informations, ks_interval_vals):
        ...
        if ks_interval_type == 'quarter':
            # ks_quarter_from, ks_quarter_to = date_utils.get_quarter(ks_end_date) METRO COMMENTED
            ks_quarter_from, ks_quarter_to = self.get_fiscal_quarter_start_end_dates(ks_df_informations, ks_interval_type, ks_end_date)
            ks_interval_type = 'quarter'
        if ks_interval_type == 'year':
            # ks_quarter_from, ks_quarter_to = date_utils.get_fiscal_year(ks_end_date) METRO COMMENTED
            company_fiscalyear_dates = self.env.company.compute_fiscalyear_dates(ks_end_date)
            ks_quarter_from, ks_quarter_to = company_fiscalyear_dates['date_from'], company_fiscalyear_dates['date_to']
            ks_interval_type = 'year'
    ...
    def ks_show_df_journal_items(self, ks_df_informations, ks_parameter=None):
        ...
        if ks_parameter and 'taxId' in ks_parameter:
            ks_tax_id = ks_parameter['taxId']
            ks_action['domain'] = str([('tax_ids','in',[ks_tax_id])])
        ...
        if ks_df_informations:
            ...
            if 'ks_filter_context' in ks_df_informations.keys():
                ks_filter_context = ks_df_informations.get('ks_filter_context')
                if 'state' in ks_filter_context.keys():
                    if ks_filter_context.get('state') == 'draft':
                        ks_ctx['search_default_unposted'] = True
                    if ks_filter_context.get('state') == 'posted':
                        ks_ctx['search_default_posted'] = True

            # if not ks_df_informations.get('all_entries'):
            #     ks_ctx['search_default_posted'] = True
            # Manage state filter
            state_domain = [('move_id.state', 'in', ['draft', 'posted'])]  # default
            if 'ks_filter_context' in ks_df_informations:
                ks_filter_context = ks_df_informations['ks_filter_context']
                if ks_filter_context.get('state') == 'draft':
                    ks_ctx['search_default_unposted'] = True
                    state_domain = [('move_id.state', '=', 'draft')]
                elif ks_filter_context.get('state') == 'posted':
                    ks_ctx['search_default_posted'] = True
                    state_domain = [('move_id.state', '=', 'posted')]
