...
class KsDynamicFinancialReportBase(models.Model):
    ...
    ks_df_report_account_type = fields.Selection([
        ...
        ('account_report', 'Report Value'),
             ...
             "'Account Type': it's the sum of leaf accoutns with such an Chart of account_type\n"
             "'Report Value': it's the amount of the related report\n"
             "'Total': it's the sum of the children of this record\n"
             "'Subtraction': it's the difference of the children of this record")
    ...
    ks_custom_report_1 = fields.Many2one('ks.dynamic.financial.reports', string='Report 1')
    ks_custom_report_2 = fields.Many2one('ks.dynamic.financial.reports', string='Report 2')
    ...
    def get_sorted_accounts(self, ks_report_lines):
        print("\n\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> get_sorted_accounts () ....account_dic ===",self, ks_report_lines)
        ks_report_lines = dict(sorted(ks_report_lines.items()))
        print("Modified ....",ks_report_lines)

        return ks_report_lines

    def get_sorted_accounts_by_name(self, ks_report_lines):
        ks_report_lines = dict(sorted(ks_report_lines.items(), key=lambda item: item[1]['name'].lower()))
        print("Sorted By Name ....",ks_report_lines)
        return ks_report_lines
