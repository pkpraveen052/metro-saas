...
class KsDynamicFinancialXlsxAR(models.Model):
    ...
    def ks_get_xlsx_Aging(self, ks_df_informations):
        ...
        line_header_light_period.set_num_format(format_string)
        ...
        address_font = workbook.add_format({
            'font_size': 10,
            'font': 'Arial',
        })
        sheet.set_row(1, 30)
        if ks_df_informations:
            ...
            address_wrap_format = workbook.add_format({'text_wrap': True, 'valign': 'top', 'font_size': 10, 'font': 'Arial',})
            sheet.merge_range(0, 4, 0, 5, self.env.company.name, company_format)
            sheet.merge_range(1, 4, 1, 5, address, address_wrap_format)
            if self.env.company.l10n_sg_unique_entity_number:
                uen = 'UEN:' + self.env.company.l10n_sg_unique_entity_number
                sheet.merge_range(2, 4, 2, 5, uen, address_font)
            if self.env.company.phone:
                phone = 'Phone:' + self.env.company.phone
                sheet.merge_range(3, 4, 3, 5, phone, address_font)
            if self.env.company.email:
                email = 'Email:' + self.env.company.email
                sheet.merge_range(4, 4, 4, 5, email, address_font)
        ...
        if partner_dict:
            for line in partner_dict:
                ...
                if ks_df_informations.get('ks_report_with_lines', False):
                    if line != 'Total':
                        count, offset, move_lines, period_list, move_line_dict = self.ks_process_aging_data(ks_df_informations,
                                                                                            ks_partner=line,
                                                                                            fetch_range=1000000)
