    ...
    def _update_user_groups_view(self):
        ...
        if self._context.get('install_filename') or self._context.get(MODULE_UNINSTALL_FLAG):
            ...
            for xml_cat in sorted(xml_by_category.keys(), key=lambda it: it[0]):
                ...
                # Custom Code starts
                if master_category_name in ["User Guide", "Administration", "Developer Tools"]:
                    self.env['res.users'].search([])._compute_is_admin()
                    admin_groups = "metroerp_userguide.manager_sub_admin_group,metroerp_userguide.admin_sub_admin_group,base.group_erp_manager,base.group_system"
