...
class AccountBankStatementLine(models.Model):
    ...
    move_line_ids = fields.Many2many('account.move.line', string='Reconcile Draft Journal Items')
    manual_operations_reconcile_line_ids = fields.Many2many(
        'manual.operations.reconcile.line',
        'st_line_reconcile_rel',  # <-- short relation table name
        'statement_line_id',  # left column
        'reconcile_line_id',  # right column
        string='Manual Operations Reconcile Lines'
    )
    # manual_operations_reconcile_line_ids = fields.Many2many('manual.operations.reconcile.line', string='Manual Operations Reconcile Line')
    ...
    @api.model
    def action_save_draft_propositions(self, st_line_id, move_line_ids, draftLines, partner_id=False): # Metro Custom 13oct25 - added the arg partner_id
        """
        this method used to
        save journal item from reconcile widget screen to bank st line on Save as Draft Button.
        """
        st_line = self.env['account.bank.statement.line'].browse(st_line_id)
        # Metro Custom 13Oct25 - added this block of code
        if partner_id:
            partner = self.env['res.partner'].sudo().browse(partner_id)
            st_line.sudo().partner_id = partner.id
        # Ends
        create_line_dicts = [
            line for line in draftLines
            if isinstance(line, dict)
               and isinstance(line.get('id'), str)
               and line['id'].startswith('createLine')
        ]
        incoming_names = set(line.get('name') for line in create_line_dicts if line.get('name'))
        to_remove = st_line.manual_operations_reconcile_line_ids.filtered(
            lambda rec: rec.name not in incoming_names
        )
        if to_remove:
            st_line.write({
                'manual_operations_reconcile_line_ids': [(3, rec.id) for rec in to_remove]
            })
        new_lines = self.env['manual.operations.reconcile.line']
        existing_names = set(st_line.manual_operations_reconcile_line_ids.mapped('name'))
        for line in create_line_dicts:
            line_name = line.get('name')
            # If name already exists, skip creating
            if line_name in existing_names:
                continue
            new_line = self.env['manual.operations.reconcile.line'].create({
                'name': line.get('name'),
                'account_id': line.get('account_id'),
                'amount': line.get('amount'),
                'amount_str': line.get('amount_str'),
                'account_code': line.get('account_code'),
                'bank_label': line.get('label')
                # 'statement_line_id': st_line.id,
            })
            new_lines |= new_line

        if new_lines:
            st_line.manual_operations_reconcile_line_ids = [(4, new.id) for new in new_lines]

        exiting_move_line_ids = [ml_id for ml_id in move_line_ids if isinstance(ml_id, int)]
        st_line.move_line_ids = [(6, 0, exiting_move_line_ids)]
        return True
    ...
    def _compute_is_reconciled(self):
        '''
        Method overide by,
        when st line is_Reconcile is true so remove move line ids
        which is draft reconcile journal item
        '''
        super(AccountBankStatementLine, self)._compute_is_reconciled()
        for st_line in self:
            if st_line.is_reconciled:
                st_line.move_line_ids = [(5, 0, 0)]