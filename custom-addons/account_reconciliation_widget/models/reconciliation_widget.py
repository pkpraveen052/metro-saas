...
class AccountReconciliation(models.AbstractModel):
    ...
    def get_bank_statement_line_data(self, st_line_ids, excluded_ids=None):
        ...
        for line in bank_statement_lines:
            ...
            if matching_amls[line.id].get("status") == "reconciled":
                ...
                if line.manual_operations_reconcile_line_ids:
                    for new_ml in line.manual_operations_reconcile_line_ids:
                        manual_line_dicts.append({
                            'id': new_ml.name,
                            'name': new_ml.bank_label,
                            'account_id': new_ml.account_id.id,
                            'amount': new_ml.amount,
                            'amount_str': new_ml.amount_str,
                            'account_code': new_ml.account_code,
                            'debit': new_ml.amount if new_ml.amount > 0 else 0,
                            'credit': -new_ml.amount if new_ml.amount < 0 else 0,
                            'label': new_ml.bank_label
                        })
                # aml_ids = matching_amls[line.id]["aml_ids"]
                aml_ids = list(set(
                    (line.move_line_ids.ids or []) +
                    (matching_amls[line.id]["aml_ids"] or [])
                ))
                ...
                line_vals = {
                    ...
                    "reconciliation_proposition": aml_ids and (
                        self._prepare_move_lines(
                            amls,
                            target_currency=target_currency,
    ...
    def get_move_lines_for_manual_reconciliation(
        ...
        domain = self._domain_move_lines_for_manual_reconciliation(line,
    ...
    def _domain_move_lines_for_reconciliation(
        ...
        # And date or due date less or equal st line date
        # line
        domain = expression.AND([domain, [("move_id.state", "=", "posted")]])
        if st_line.statement_id:
            domain = expression.AND([domain, ['|',
                                               ('date_maturity', '<=', st_line.statement_id.date),
                                               '&',
                                               ('date_maturity', '=', False),
        ...
        if lock_date:
            domain = expression.AND([domain, [("move_id.date", ">", lock_date)]])
    ...
    def _domain_move_lines_for_manual_reconciliation(
        self, line, account_id, partner_id=False, excluded_ids=None, search_str=False
    ):
        ...
        if line.statement_id:
            domain = expression.AND([domain, ['|',
                                               ('date_maturity', '<=', line.statement_id.date),
                                               '&',
                                               ('date_maturity', '=', False),
