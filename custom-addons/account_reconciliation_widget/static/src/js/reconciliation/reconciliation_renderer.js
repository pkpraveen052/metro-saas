odoo.define("account.ReconciliationRenderer", function (require) {
    ...
    var LineRenderer = Widget.extend(FieldManagerMixin, {
        ...
        events: {
            ...
            "click .o_save_draft": "_onSaveDraftClick",
            keydown: "_onKeydown",
        },
        ...
        _makePartnerRecord: function (partnerID, partnerName) {
            ...
            return this.model.makeRecord("account.bank.statement.line", [field], {
                partner_id: {
                    domain: [["company_id", "in", [false, this._initialState.st_line.company_id]],"|", ["is_company", "=", true], ["parent_id", "=", false]],
                    options: {
                        no_open: true,
                    },
                },
        ...
        _onValidate: function () {
            $('.o_save_draft').addClass('d-none'); //hide Save as Draft button on validate.
            this.trigger_up("validate");
        },
        /**
         * @on draft save move line
         store journal item(move line) into bank st line on (save as draft) button.
         */
         _onSaveDraftClick: function (ev) {
            ev.preventDefault();
            const self = this;
            const st_line_id = $(ev.currentTarget).data('st-line-id');
            // Collect selected move line IDs from reconciliation_proposition
            const selectedMoveLineIds = this._initialState.reconciliation_proposition
                .filter(l => l.display && l.id)
                .map(l => l.id);
            const draftLines = this._initialState.reconciliation_proposition
            .filter(l => l.display) // only visible lines
            .map(l => ({
                id: l.id || false, // move line id if exists
                name: l.id || '',
                account_id: l.account_id ? l.account_id.id : false,
                amount: l.amount || 0,
                amount_str: l.amount_str || false,
                account_code: l.account_code || false,
                label: l.label || false

            }));
            if (selectedMoveLineIds.length === 0) {
                this.do_warn("No lines selected", "Please select at least one move line.");
                return;
            }

            this._rpc({
                model: 'account.bank.statement.line',
                method: 'action_save_draft_propositions',
                args: [st_line_id, selectedMoveLineIds, draftLines],
                kwargs: {
                    partner_id: this._initialState.reconciliation_proposition[0].partner_id
                        ? this._initialState.reconciliation_proposition[0].partner_id.id
                        : false,
                }, // Metro Custom 13Oct25 - added this new line
            }).then(function () {
                self.do_notify("Draft Saved", "Selected move lines have been saved.");
            }).catch(function (err) {
                console.error(err);
                self.do_warn("Error", "Could not save draft.");
            });
        }, //end
