odoo.define("account.ReconciliationModel", function (require) {
    ...
    var StatementModel = BasicModel.extend({
        ...
        addProposition: function (handle, mv_line_id) {
            ...
            // savs as draft button hide/unhide on line
            if (line.reconciliation_proposition.length > 0) {
                $('.o_save_draft').removeClass('d-none');
            } else {
                $('.o_save_draft').addClass('d-none');
            }
        ...
        reload: function () {
            ...
            var def_statement = this._rpc({
                ...
                if(statement.statement_id && statement.statement_name == false){
                    self.do_action({
                        name: "Bank Statements",
                        res_model: "account.bank.statement",
                        res_id: parseInt(statement.statement_id),
                        views: [[false, "form"]],
