    ...
    def action_open_whatsapp_composer(self):
        ...
        if self._name in self.REPORT_MAPPING:
            try:
                ...
                if self.state == 'draft':  # Check if the invoice is in draft state
                    if self.move_type == 'out_invoice': 
                        file_name = "Draft_Invoice.pdf"
                    elif self.move_type == 'in_invoice':  
                        file_name = "Draft_Bill.pdf"
                    elif self.move_type == 'in_refund':  
                        file_name = "Draft_Credit_Note.pdf"
                    elif self.move_type == 'out_refund':  
                        file_name = "Draft_Credit_Note.pdf"
                    else:
                        file_name = "Draft_Document.pdf"  # Fallback (if any other case exists)
                else:
