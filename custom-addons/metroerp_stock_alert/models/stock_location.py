    ...
    def _cron_send_alert_for_low_stock(self):
        ...
        def _set_html_body(low_stock_quant_ids):
            ...
            for low_stock_quant_id in low_stock_quant_ids:
                ...
                """.format(
                    product_name=low_stock_quant_id.display_name,
        ...
        self = self.sudo()
        group_id = self.env.ref('metroerp_stock_alert.group_notify_low_stock')
        ...
        for company_id in company_ids:
            ...
            # Only users belonging to this company get alerts
            company_user_ids = group_id.users.filtered(
                lambda u: u.company_id.id == company_id.id
            )
            ...
            for internal_loc_id in internal_loc_ids:
                ...
                low_stock_quant_ids = internal_loc_id.quant_ids.filtered(
                    lambda quant: quant.available_quantity <= low_stock_alert_qty
                )

                if not low_stock_quant_ids:
                    continue

                html_body = _set_html_body(low_stock_quant_ids)

                # Loop only company users (FIXED)
