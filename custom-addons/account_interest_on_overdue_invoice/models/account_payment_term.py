...
class AccountPaymentTerms(models.Model):
    ...
    interest_type = fields.Selection(
        string='Type (Simple Interest)', help="Base duration for interest calculation",
        selection=[('daily', 'Daily'),
                   ('weekly', 'Weekly'),
                   ('monthly', 'Monthly')],
        default='daily', copy=False)
    interest_percentage = fields\
        .Float(string="Percentage", default="0", digits=(12, 6),
    ...
    interest_account_id = fields.Many2one('account.account', string='Account '
                                          ...
                                          help="Account used for creating "
                                               "accounting entries.")
    interest_basis = fields.Selection([('tax_inclusive', 'Tax Inclusive'), ('tax_exclusive', 'Tax Exclusive')], default='tax_inclusive', string='Interest Basis')
