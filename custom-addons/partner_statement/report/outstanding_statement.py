...
class OutstandingStatement(models.AbstractModel):
    ...
    def _display_outstanding_lines_sql_q1(self, partners, date_end, account_type):
        ...
        return str(
            ...
            SELECT l.id, m.name AS move_id, l.partner_id, l.date, l.name,
                l.blocked, l.currency_id, l.company_id, m.initial_carry_fwd_balance,
            ...
            WHERE l.partner_id IN %(partners)s AND at.type = %(account_type)s
                ...
                ) AND l.date <= %(date_end)s AND m.state IN ('posted') AND m.adjust_in_carry_fwd_bal IS NOT TRUE
            GROUP BY l.id, l.partner_id, m.name, l.date, l.date_maturity, l.name, m.initial_carry_fwd_balance,
                CASE WHEN l.ref IS NOT NULL
                    THEN l.ref
                    ELSE m.ref
                END,
            ...
            self._cr.mogrify(
                f"""
                   SELECT {sub}.partner_id, {sub}.currency_id, {sub}.move_id,
                       ...
                       {sub}.name, {sub}.ref, {sub}.blocked, {sub}.company_id, {sub}.initial_carry_fwd_balance,
        ...
        return str(
            self._cr.mogrify(
               ...
               SELECT {sub}.partner_id, {sub}.move_id, {sub}.date,
                   ...
                   {sub}.open_amount, {sub}.blocked, {sub}.id, {sub}.initial_carry_fwd_balance
               FROM {sub}
    ...
    def _get_account_display_lines(
        ...
        SELECT partner_id, currency_id, move_id, date, date_maturity, debit,
            ...
            COALESCE(ref, '') as ref, blocked, id, initial_carry_fwd_balance
        FROM Q3
