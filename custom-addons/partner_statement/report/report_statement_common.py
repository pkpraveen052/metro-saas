...
class ReportStatementCommon(models.AbstractModel):
        ...
        return str(
            ...
            WHERE l.partner_id IN %(partners)s AND at.type = %(account_type)s
                                AND (
                                  ...
                                  AND m.state IN ('posted') AND m.adjust_in_carry_fwd_bal IS NOT TRUE
    ...
    # def _get_initial_carry_fwd_balance(self, partner_id, date_end):
    #     self.env.cr.execute(
    #         """
    #         SELECT l.id, l.date, l.amount_currency, l.balance, l.currency_id
    #         FROM account_move_line l
    #         JOIN account_move m ON l.move_id = m.id
    #         WHERE l.partner_id = %s AND m.initial_carry_fwd_balance = TRUE AND l.date <= %s AND balance > 0
    #         """,
    #         (partner_id, date_end)
    #     )
    #     return self.env.cr.dictfetchall()

    # def _adjust_buckets_with_carry_fwd(self, buckets, partner_id, carry_fwd_entries, full_dates,date_end):
    #     for entry in carry_fwd_entries:
    ...
    def _get_account_show_buckets(
        ...
        for row in self.env.cr.dictfetchall():
            ...
            statement_type = 'customer' if account_type == 'receivable' else 'supplier'
            inital_carry_forward_balances = self.env['account.move.line'].search([
                ('partner_id', '=', partner_id),
                ('move_id.initial_carry_fwd_balance', '=', True),
                ('balance', '<', 0),
                ('date', '<=', date_end),
                ('move_id.statement_type', '=', statement_type),
                ('move_id.state', '=', 'posted')
            ])
            for inital_carry_forward_balance in inital_carry_forward_balances:
                if inital_carry_forward_balance:
                    if account_type == 'receivable':
                        adjustment_lines = self.env['account.move.line'].search([
                            ('partner_id', '=', partner_id),
                            ('move_id.adjust_in_carry_fwd_bal', '=', True),
                            ('balance', '<', 0),
                            ('date', '<=', date_end),
                        ...
                        adjustment_lines = self.env['account.move.line'].search([
                            ...
                            ('balance', '>', 0),
                            ('date', '<=', date_end),
                            ('move_id.payment_id.payment_type', '=', 'outbound'),
                            ('move_id.state', '=', 'posted')
                        ])
                    amount_to_deduct = sum(adjustment.balance for adjustment in adjustment_lines)
                    if amount_to_deduct:
                        if inital_carry_forward_balance.date >= full_dates['minus_30']:
                            row['b_1_30'] += amount_to_deduct
                        elif full_dates['minus_30'] > inital_carry_forward_balance.date >= full_dates['minus_60']:
                            row['b_30_60'] += amount_to_deduct
                        elif full_dates['minus_60'] > inital_carry_forward_balance.date >= full_dates['minus_90']:
                            row['b_60_90'] += amount_to_deduct
                        elif full_dates['minus_90'] > inital_carry_forward_balance.date >= full_dates['minus_120']:
                            row['b_90_120'] += amount_to_deduct
                        elif inital_carry_forward_balance.date <= full_dates['minus_120']:
                            row['b_over_120'] += amount_to_deduct

                    total = row['current']+row['b_1_30']+row['b_30_60']+row['b_60_90']+row['b_90_120']+row['b_over_120']
                    row['balance'] = total
            buckets[partner_id].append(row)
        return buckets
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if self._description == "Partner Outstanding Statement":
            for partner_id, entries in lines.items():
                for entry_line in entries:
                    if entry_line["date_maturity"] > date_end:
                        not_yet_due += entry_line["amount"]
                initial_entries = [entry for entry in entries if entry.get('initial_carry_fwd_balance')]
                statement_type = 'customer' if account_type == 'receivable' else 'supplier'
                if account_type == 'receivable':
                    adjustment_lines = self.env['account.move.line'].search([
                        ('partner_id', '=', partner_id),
                ...
                total_adjustment = sum(adjustment.balance for adjustment in adjustment_lines)
                if initial_entries and total_adjustment:
                    for initial_entry in initial_entries:
                        initial_entry['open_amount'] += total_adjustment

                filtered_entries = [entry for entry in entries if abs(entry['open_amount']) > 1e-10]
                lines[partner_id] = filtered_entries
