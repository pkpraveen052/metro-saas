...
class ResPartner(models.Model):
        ...
        for partner in self:
            if partner.active:
                partners_to_email = [child for child in partner.child_ids if
                                 child.type == 'invoice' and child.email and child.active]
            ...
            if partners_to_email:
                ...
                for partner_to_email in partners_to_email:
                    if level and level.send_email and \
                        ...
                        level.email_template_id.with_context(ctx).with_company(company).send_mail(
                            partner_to_email.id,force_send=True)
                    else:
                        mail_template_id = self.env.ref(template)
                        company = partner.company_id
                        mail_template_id.with_context(ctx).with_company(company).send_mail(
                            partner_to_email.id,force_send=True)
    ...
    def get_company_bank_details(self):
        company = self.env.company  # current logged-in company
        ...
        if journal and journal.bank_account_id:
            bank_name = journal.bank_account_id.bank_id.name or ''
            acc_number = journal.bank_account_id.acc_number or ''
