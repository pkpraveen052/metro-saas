...
class DynamicFields(models.Model):
    ...
    group_id = fields.Many2one('dynamic.field.group', string='Field Group')
    field_added_form_view = fields.Boolean(string="Field Added In Form View")
    ...
    def _onchange_template_id(self):
        """
        Check if the selected template_id matches the group_id's template_id.
        If not, raise a UserError warning.
        """
        if self.template_id and self.group_id and self.template_id != self.group_id.template_id:
            raise UserError("The selected Service Template must match the template of the Group.")
    ...
    def action_create_dynamic_fields(self):
        """ The 'CREATE FIELD' button method is used to add a new field or group dynamically
            without directly modifying the base view, to preserve changes after upgrades."""

        # Update the status to form
        self.write({'status': 'form'})

        # Ensure 'currency_id' exists for monetary fields
        if self.field_type == 'monetary' and not self.env['ir.model.fields'].sudo().search(
    ...
    def action_add_field_to_form_view(self):
        # Define the inherit view name for each new field/group
        ...
        # Handle default dynamic field and position if not provided
        default_field = 'name'  # Fallback field
        ref_field = self.dynamic_field_id.name if self.dynamic_field_id else default_field
        default_position = 'after' if not self.position else self.position

        # Handle the widget if provided
        widget_attribute = f' widget="{self.widget_id.name}"' if self.widget_id else ""

        # Handle group addition or field placement inside existing group
        group_exists = self._check_group_exists(inherit_id, self.group_id)
        if self.group_id and group_exists:
            # Append the new field to the existing group using XPath
            arch_base = _(
                '<?xml version="1.0"?>'
                '<data>'
                '<xpath expr="//group[@name=\'%s\']" position="inside" attrs="{\'invisible\': [(\'template_id\', \'!=\', %d)]}">'
                '<field name="%s"%s attrs="{\'invisible\': [(\'template_id\', \'!=\', %d)]}"/>'
                '</xpath>'
                '</data>') % (self.group_id.name, self.group_id.template_id.id,self.name, widget_attribute, self.group_id.template_id.id)
        elif self.group_id:
            # Create a new group and add the field

            last_group_name = self._get_last_group_name(inherit_id)  # Fetch the last group name in the view

            arch_base = _(
                '<?xml version="1.0"?>'
                '<data>'
                '<xpath expr="//group[@name=\'%s\']" position="after">'
                '<group name="%s" string="%s" attrs="{\'invisible\': [(\'template_id\', \'!=\', %d)]}">'
                '<field name="%s"%s attrs="{\'invisible\': [(\'template_id\', \'!=\', %d)]}"/>'
                '</group>'
                '</xpath>'
                '</data>') % (last_group_name, self.group_id.name, self.group_id.group_string,self.template_id.id, self.name,
                              widget_attribute, self.template_id.id)
        else:
            # Just add the new field after the reference field
            arch_base = _('<?xml version="1.0"?>'
                          '<data>'
                          '<field name="%s" position="%s">'
                          '<field name="%s"%s attrs="{\'invisible\': [(\'template_id\', \'!=\', %d)]}"/>'
                          '</field>'
                          '</data>') % (ref_field, default_position, self.name, widget_attribute, self.template_id.id)

        # Create an inherited view with dynamic changes
        ...
        self.field_added_form_view = True
    ...
    def _get_last_group_name(self, inherit_id):
        """Helper method to get the last group name in the form view."""
        form_views = self.env['ir.ui.view'].search([('model', '=', 'service.management'), ('type', '=', 'form')])
        last_group_name = None
        for form_view in form_views:
            groups = form_view.arch_db.split('<group name="')  # Split the arch XML by group name tags
            if len(groups) > 1:
                last_group_name = groups[-1].split('"')[0]  # Get the last group name in the list
    ...
    def _check_group_exists(self, inherit_id, group):
        """Helper method to check if a group already exists in the form view."""
        # Search the form view for the group
        group_exists = False
        form_views = self.env['ir.ui.view'].search([('model', '=', 'service.management'), ('type', '=', 'form')])
        for form in form_views:
            if f'<group name="{group.name}"' in form.arch_db:
                group_exists = True
        return group_exists

    def _check_page_exists(self, page_name):
        """Helper method to check if a page (tab) already exists in the form view."""
        page_exists = False
        form_views = self.env['ir.ui.view'].search([('model', '=', 'service.management'), ('type', '=', 'form')])
        for form in form_views:
            if f'<page name="{page_name}"' in form.arch_db:
                page_exists = True
                break
        return page_exists