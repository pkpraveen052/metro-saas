    ...
    def button_sync_statement_lines(self):
        for statement in self:
            ...
            for move in unreconciled_moves_outstanding_receipt:
                move_name = move.name if move.name else ''

                payment_ref = " ".join(filter(None, [move_name, move.ref]))
                statement_line = self.env['account.bank.statement.line'].with_context(active_test=False).search([('name', '=', move_name), ('partner_id', '=', move.partner_id.id),
